"use strict";

// =============================================================
// EventBus — minimal pub/sub base class
// =============================================================

class EventBus {
  constructor() {
    this.listeners = {};
  }

  on(eventName, callback) {
    if (!this.listeners[eventName]) {
      this.listeners[eventName] = [];
    }
    this.listeners[eventName].push(callback);
    return this;
  }

  off(eventName, callback) {
    if (!this.listeners[eventName]) {
      return this;
    }
    this.listeners[eventName] = this.listeners[eventName].filter(
      (fn) => fn !== callback,
    );
    return this;
  }

  emit(eventName, ...args) {
    (this.listeners[eventName] || []).forEach((fn) => fn(...args));
    return this;
  }
}

// =============================================================
// PathPoint — a single movable bend point on an edge's path
// =============================================================

class PathPoint extends EventBus {
  constructor(parentEdge, pointIndex) {
    super();
    this.id = `pathpoint-${Math.random().toString(36).slice(2)}`;
    this.edge = parentEdge;
    this.pointIndex = pointIndex;
  }

  get x() {
    return this.edge.vertices()[this.pointIndex]?.x ?? 0;
  }

  get y() {
    return this.edge.vertices()[this.pointIndex]?.y ?? 0;
  }

  moveTo(x, y) {
    const points = [...this.edge.vertices()];
    points[this.pointIndex] = { x, y };
    this.edge.vertices(points);
    this.emit("move", this);
    return this;
  }

  moveBy(deltaX, deltaY) {
    return this.moveTo(this.x + deltaX, this.y + deltaY);
  }

  remove() {
    const points = [...this.edge.vertices()];
    points.splice(this.pointIndex, 1);
    this.edge.vertices(points);
    this.emit("remove", this);
  }
}

// =============================================================
// Arrow marker definitions for edge endpoints
// =============================================================

const ARROW_MARKERS = {
  none: null,
  classic: { type: "path", d: "M 10 -5 0 0 10 5 Z" },
  block: { type: "path", d: "M 10 -10 0 0 10 10 Z" },
};

// =============================================================
// Edge — a connection between two nodes
// =============================================================

class Edge extends EventBus {
  constructor(link, sourceNode, targetNode, editor) {
    super();
    this.link = link;
    this.sourceNode = sourceNode;
    this.targetNode = targetNode;
    this.editor = editor;
    this.pathPoints = [];
  }

  get id() {
    return this.link.id;
  }

  get source() {
    return this.sourceNode;
  }

  get target() {
    return this.targetNode;
  }

  // label

  get label() {
    return this.link.label(0)?.attrs?.text?.text || "";
  }

  set label(value) {
    this._applyLabel(value);
    this.emit("change", this);
  }

  get labelColor() {
    return this.link.label(0)?.attrs?.text?.fill || "#333333";
  }

  set labelColor(value) {
    this.link.prop("labelColor", value);
    this._applyLabel(this.label);
    this.emit("change", this);
  }

  get labelFontSize() {
    return this.link.get("fontSizePercent") || 100;
  }

  set labelFontSize(value) {
    this.link.set("fontSizePercent", value);
    this._applyLabel(this.label);
    this.emit("change", this);
  }

  // line appearance

  get lineColor() {
    return this.link.attr("line/stroke") || "#495057";
  }

  set lineColor(value) {
    this.link.attr("line/stroke", value);
    this.emit("change", this);
  }

  get lineWidth() {
    return this.link.attr("line/strokeWidth") || 2;
  }

  set lineWidth(value) {
    this.link.attr("line/strokeWidth", value);
    this.emit("change", this);
  }

  get lineStyle() {
    const dash = this.link.attr("line/strokeDasharray") || "";
    if (dash === "5,5") {
      return "dashed";
    }
    if (dash === "1,5") {
      return "dotted";
    }
    return "solid";
  }

  set lineStyle(value) {
    if (value === "dashed") {
      this.link.attr("line/strokeDasharray", "5,5");
    } else if (value === "dotted") {
      this.link.attr("line/strokeDasharray", "1,5");
    } else {
      this.link.attr("line/strokeDasharray", "");
    }
    this.emit("change", this);
  }

  // arrows

  get sourceArrow() {
    return this._arrowNameFromDefinition(this.link.attr("line/sourceMarker"));
  }

  set sourceArrow(value) {
    this.link.attr("line/sourceMarker", ARROW_MARKERS[value] || null);
    this.emit("change", this);
  }

  get targetArrow() {
    return this._arrowNameFromDefinition(this.link.attr("line/targetMarker"));
  }

  set targetArrow(value) {
    this.link.attr("line/targetMarker", ARROW_MARKERS[value] || null);
    this.emit("change", this);
  }

  // connector routing

  get connectorType() {
    const connector = this.link.connector()?.name;
    const router = this.link.router()?.name;
    if (connector === "smooth") {
      return "curved";
    }
    if (router === "normal") {
      return "straight";
    }
    return "elbow";
  }

  set connectorType(value) {
    const gridSize = this.editor.gridSize;
    if (value === "elbow") {
      this.link.router("manhattan", { step: gridSize, padding: 20 });
      this.link.connector("rounded");
    } else if (value === "straight") {
      this.link.router("normal");
      this.link.connector("normal");
    } else {
      this.link.router("normal");
      this.link.connector("smooth");
    }
    this.emit("change", this);
  }

  // port pinning

  get sourcePort() {
    return this.link.get("sourcePort") ?? null;
  }

  set sourcePort(value) {
    this._setPort("source", value);
  }

  get targetPort() {
    return this.link.get("targetPort") ?? null;
  }

  set targetPort(value) {
    this._setPort("target", value);
  }

  // description

  get description() {
    return this.link.get("description") || "";
  }

  set description(value) {
    this.link.set("description", value);
    this.emit("change", this);
  }

  // path points (bend points along the edge)

  getPathPoints() {
    return [...this.pathPoints];
  }

  addPathPoint(x, y) {
    const points = [...this.link.vertices(), { x, y }];
    this.link.vertices(points);

    const point = new PathPoint(this.link, points.length - 1);
    this.pathPoints.push(point);
    return point;
  }

  // actions

  select() {
    this.editor._selectItem(this);
    return this;
  }

  deselect() {
    this.editor.clearSelection();
    return this;
  }

  remove() {
    this.link.remove();
    this.emit("remove", this);
  }

  // internal helpers

  _applyLabel(text) {
    const fontSize = 12 * ((this.link.get("fontSizePercent") || 100) / 100);
    const color = this.link.get("labelColor") || "#333333";

    if (text) {
      this.link.labels([
        {
          attrs: {
            text: { text, fill: color, fontSize, textVerticalAnchor: "middle" },
            rect: { fill: "white", opacity: 0.9 },
          },
          position: { distance: 0.5 },
        },
      ]);
    } else {
      this.link.labels([]);
    }
  }

  _arrowNameFromDefinition(definition) {
    if (!definition) {
      return "none";
    }
    if (definition.d === ARROW_MARKERS.classic.d) {
      return "classic";
    }
    if (definition.d === ARROW_MARKERS.block.d) {
      return "block";
    }
    return "none";
  }

  _setPort(side, portIndex) {
    const node = side === "source" ? this.sourceNode : this.targetNode;
    this.link.set(`${side}Port`, portIndex);

    if (portIndex === null) {
      this.link[side]({ id: node.cell.id });
    } else {
      const port = node.cell.getPorts()[portIndex - 1];
      if (port) {
        this.link[side]({ id: node.cell.id, port: port.id });
      }
    }
    this.emit("change", this);
  }
}

// =============================================================
// Node — base class for a diagram shape
// =============================================================

class Node extends EventBus {
  constructor(options = {}) {
    super();
    this._label = typeof options === "string" ? options : (options.label ?? "");
    this._initOptions = typeof options === "string" ? {} : options;
    this.cell = null;
    this.editor = null;
    this.schema = {};
    this.customProps = {};
    this.renderFn = null;
  }

  // custom properties

  getCustomProperty(key) {
    return this.customProps[key];
  }

  setCustomProperty(key, value) {
    this.customProps[key] = value;
    this.cell?.set(`custom_${key}`, value);
    if (this.renderFn) {
      this.renderFn(this);
    }
    this.emit("change", this);
  }

  getSchema() {
    return this.schema;
  }

  // position and size

  get id() {
    return this.cell?.id ?? null;
  }

  get x() {
    return this.cell?.position().x ?? 0;
  }

  set x(value) {
    this.cell?.position(value, this.y);
    this.emit("change", this);
  }

  get y() {
    return this.cell?.position().y ?? 0;
  }

  set y(value) {
    this.cell?.position(this.x, value);
    this.emit("change", this);
  }

  get width() {
    return this.cell?.size().width ?? 0;
  }

  get height() {
    return this.cell?.size().height ?? 0;
  }

  get portCount() {
    return 4;
  }

  // label

  get label() {
    return this.cell?.attr("label/text") ?? this._label;
  }

  set label(value) {
    this.cell?.attr("label/text", value);
    this._label = value;
    this._resizeToFitContent();
    this.emit("change", this);
  }

  get labelColor() {
    return this.cell?.attr("label/fill") ?? "#212529";
  }

  set labelColor(value) {
    this.cell?.attr("label/fill", value);
    this.emit("change", this);
  }

  get labelFontSize() {
    return this.cell?.get("fontSizePercent") ?? 100;
  }

  set labelFontSize(value) {
    this.cell?.set("fontSizePercent", value);
    this._resizeToFitContent();
    this.emit("change", this);
  }

  // description

  get description() {
    return this.cell?.get("description") ?? "";
  }

  set description(value) {
    this.cell?.set("description", value);
    this.cell?.attr("descriptionLabel/text", value);
    this._resizeToFitContent();
    this.emit("change", this);
  }

  get descriptionColor() {
    return this.cell?.attr("descriptionLabel/fill") ?? "#6c757d";
  }

  set descriptionColor(value) {
    this.cell?.attr("descriptionLabel/fill", value);
    this.emit("change", this);
  }

  // appearance

  get backgroundColor() {
    return this.cell?.attr("body/fill") ?? "#ffffff";
  }

  set backgroundColor(value) {
    this.cell?.attr("body/fill", value);
    this.emit("change", this);
  }

  get borderColor() {
    return this.cell?.attr("body/stroke") ?? "#adb5bd";
  }

  set borderColor(value) {
    this.cell?.attr("body/stroke", value);
    this.emit("change", this);
  }

  get borderWidth() {
    return this.cell?.attr("body/strokeWidth") ?? 2;
  }

  set borderWidth(value) {
    this.cell?.attr("body/strokeWidth", value);
    this.emit("change", this);
  }

  // image

  get imageUrl() {
    return this.cell?.get("imageUrl") ?? "";
  }

  set imageUrl(value) {
    this.cell?.set("imageUrl", value);
    this.cell?.attr("image/xlink:href", value);
    this.cell?.attr("image/display", value ? "block" : "none");
    this._resizeToFitContent();
    this.emit("change", this);
  }

  get imageWidth() {
    return this.cell?.get("imageWidth") ?? 32;
  }

  set imageWidth(value) {
    this.cell?.set("imageWidth", value);
    this._resizeToFitContent();
    this.emit("change", this);
  }

  get imageHeight() {
    return this.cell?.get("imageHeight") ?? 32;
  }

  set imageHeight(value) {
    this.cell?.set("imageHeight", value);
    this._resizeToFitContent();
    this.emit("change", this);
  }

  // status and priority

  get status() {
    return this.cell?.get("status") ?? "pending";
  }

  set status(value) {
    this.cell?.set("status", value);
    this.emit("change", this);
  }

  get priority() {
    return this.cell?.get("priority") ?? 1;
  }

  set priority(value) {
    this.cell?.set("priority", value);
    this.emit("change", this);
  }

  // movement

  moveTo(x, y) {
    this.cell?.position(x, y);
    this.emit("move", this);
    return this;
  }

  moveBy(deltaX, deltaY) {
    return this.moveTo(this.x + deltaX, this.y + deltaY);
  }

  toFront() {
    this.cell?.toFront();
    return this;
  }

  toBack() {
    this.cell?.toBack();
    return this;
  }

  // selection

  select() {
    this.editor?._selectItem(this);
    return this;
  }

  deselect() {
    this.editor?.clearSelection();
    return this;
  }

  // edges

  getEdges() {
    return this.editor?._getEdgesForNode(this) ?? [];
  }

  getIncomingEdges() {
    return this.getEdges().filter((edge) => edge.target === this);
  }

  getOutgoingEdges() {
    return this.getEdges().filter((edge) => edge.source === this);
  }

  connect(targetNode, sourcePortIndex = null, targetPortIndex = null) {
    if (targetNode === this) {
      throw new Error("A node cannot connect to itself.");
    }
    return this.editor?._createEdge(
      this,
      targetNode,
      sourcePortIndex,
      targetPortIndex,
    );
  }

  // lifecycle

  remove() {
    this.cell?.remove();
    this.emit("remove", this);
  }

  _buildCell(position, jointNamespace) {
    throw new Error("_buildCell must be implemented by subclass");
  }

  _resizeToFitContent() {
    if (!this.editor || !this.cell) {
      return;
    }
    this.editor._fitNodeToContent(this.cell);
  }

  _getShapeType() {
    return "rect";
  }
}

// =============================================================
// Node.define — factory that creates custom node classes
// =============================================================

Node.define = function (
  BaseNodeClass,
  defaultOptions = {},
  schema = {},
  renderFn = null,
) {
  class CustomNode extends BaseNodeClass {
    constructor(options = {}) {
      super();
      this.customProps = {};
      this.schema = schema;
      this.renderFn = renderFn;
      this._defaultOptions = defaultOptions;

      // merge default options with instance options
      const merged = { ...defaultOptions, ...options };

      // copy any built-in property overrides so addNode() can apply them later
      const builtIns = [
        "label",
        "labelColor",
        "labelFontSize",
        "description",
        "descriptionColor",
        "backgroundColor",
        "borderColor",
        "borderWidth",
        "imageUrl",
        "imageWidth",
        "imageHeight",
        "status",
        "priority",
      ];
      builtIns.forEach((key) => {
        if (merged[key] !== undefined) {
          this[`_init_${key}`] = merged[key];
        }
      });
      this._label = merged.label ?? defaultOptions.label ?? "";

      // initialise custom property values from schema defaults
      Object.entries(schema).forEach(([key, fieldDefinition]) => {
        this.customProps[key] =
          options[key] !== undefined ? options[key] : fieldDefinition.default;
      });
    }

    getCustomProperty(key) {
      return this.customProps[key];
    }

    setCustomProperty(key, value) {
      if (!(key in this.schema)) {
        throw new Error(`Unknown custom property: ${key}`);
      }
      this.customProps[key] = value;
      this.cell?.set(`custom_${key}`, value);
      if (this.renderFn) {
        this.renderFn(this);
      }
      this.emit("change", this);
    }

    getSchema() {
      return this.schema;
    }

    _buildCell(position, jointNamespace) {
      return super._buildCell(position, jointNamespace);
    }
  }

  CustomNode.prototype._getShapeType = BaseNodeClass.prototype._getShapeType;
  Object.defineProperty(
    CustomNode.prototype,
    "portCount",
    Object.getOwnPropertyDescriptor(BaseNodeClass.prototype, "portCount") || {
      get() {
        return 4;
      },
    },
  );
  return CustomNode;
};

// =============================================================
// Concrete node shape subclasses
// =============================================================

class Rectangle extends Node {
  get portCount() {
    return 4;
  }

  _getShapeType() {
    return "rect";
  }

  _buildCell(position, namespace) {
    return _buildRectangleCell(position, namespace, "rect", 140, 50);
  }
}

class Square extends Node {
  get portCount() {
    return 4;
  }

  _getShapeType() {
    return "square";
  }

  _buildCell(position, namespace) {
    return _buildRectangleCell(position, namespace, "square", 80, 80);
  }
}

class Ellipse extends Node {
  get portCount() {
    return 4;
  }

  _getShapeType() {
    return "ellipse";
  }

  _buildCell(position, namespace) {
    return _buildEllipseCell(position, namespace, "ellipse", 140, 50);
  }
}

class Circle extends Node {
  get portCount() {
    return 4;
  }

  _getShapeType() {
    return "circle";
  }

  _buildCell(position, namespace) {
    return _buildEllipseCell(position, namespace, "circle", 80, 80);
  }
}

class Diamond extends Node {
  get portCount() {
    return 4;
  }

  _getShapeType() {
    return "diamond";
  }

  _buildCell(position, namespace) {
    return _buildPolygonCell(position, namespace, "diamond", 80, 80, [
      [0, 10],
      [10, 0],
      [20, 10],
      [10, 20],
    ]);
  }
}

class Triangle extends Node {
  get portCount() {
    return 3;
  }

  _getShapeType() {
    return "triangle";
  }

  _buildCell(position, namespace) {
    return _buildPolygonCell(position, namespace, "triangle", 140, 50, [
      [10, 0],
      [20, 20],
      [0, 20],
    ]);
  }
}

class Hexagon extends Node {
  get portCount() {
    return 6;
  }

  _getShapeType() {
    return "hexagon";
  }

  _buildCell(position, namespace) {
    return _buildPolygonCell(position, namespace, "hexagon", 140, 50, [
      [5, 0],
      [15, 0],
      [20, 10],
      [15, 20],
      [5, 20],
      [0, 10],
    ]);
  }
}

class Pentagon extends Node {
  get portCount() {
    return 5;
  }

  _getShapeType() {
    return "pentagon";
  }

  _buildCell(position, namespace) {
    return _buildPolygonCell(position, namespace, "pentagon", 140, 50, [
      [10, 0],
      [20, 7],
      [16, 20],
      [4, 20],
      [0, 7],
    ]);
  }
}

class Octagon extends Node {
  get portCount() {
    return 8;
  }

  _getShapeType() {
    return "octagon";
  }

  _buildCell(position, namespace) {
    return _buildPolygonCell(position, namespace, "octagon", 140, 50, [
      [6, 0],
      [14, 0],
      [20, 6],
      [20, 14],
      [14, 20],
      [6, 20],
      [0, 14],
      [0, 6],
    ]);
  }
}

// =============================================================
// Shared JointJS cell construction helpers
// =============================================================

// markup shared by every node shape (image, label, description)
const SHARED_CELL_MARKUP = [
  { tagName: "image", selector: "image", className: "image" },
  { tagName: "text", selector: "label", className: "label" },
  {
    tagName: "text",
    selector: "descriptionLabel",
    className: "descriptionLabel",
  },
];

// default visual attributes for those markup elements
const SHARED_CELL_ATTRS = {
  image: { display: "none" },
  label: {
    fill: "#212529",
    fontSize: 13,
    fontWeight: "bold",
    pointerEvents: "none",
  },
  descriptionLabel: {
    text: "",
    fill: "#6c757d",
    fontSize: 11,
    pointerEvents: "none",
  },
};

// default model properties stored on every cell
const SHARED_CELL_DEFAULTS = {
  description: "",
  status: "pending",
  priority: 1,
  imageUrl: "",
  imageWidth: 32,
  imageHeight: 32,
  fontSizePercent: 100,
};

function _attachPortsToCell(cell, polygonPoints) {
  // all ports share the same visual style
  cell.prop("ports/groups", {
    all: {
      position: "absolute",
      attrs: {
        circle: {
          r: 6,
          fill: "#3498db",
          magnet: true,
          stroke: "#fff",
          strokeWidth: 2,
          pointerEvents: "all",
        },
        text: { display: "none" },
      },
    },
  });

  // for rectangles and ellipses, place ports at the four cardinal edges.
  // for polygons, place one port at the midpoint of each side.
  let ports;
  if (polygonPoints.length === 0) {
    ports = [
      { group: "all", args: { x: "50%", y: "0%", dy: -1 } },
      { group: "all", args: { x: "100%", y: "50%" } },
      { group: "all", args: { x: "50%", y: "100%", dy: 1 } },
      { group: "all", args: { x: "0%", y: "50%" } },
    ];
  } else {
    ports = polygonPoints.map((point, i) => {
      const next = polygonPoints[(i + 1) % polygonPoints.length];
      return {
        group: "all",
        args: {
          x: `${((point[0] + next[0]) / 2 / 20) * 100}%`,
          y: `${((point[1] + next[1]) / 2 / 20) * 100}%`,
        },
      };
    });
  }

  cell.addPorts(ports);
}

function _buildRectangleCell(position, namespace, shapeType, width, height) {
  const cell = new namespace.standard.Rectangle();
  cell.markup = [
    { tagName: "rect", selector: "body", className: "body" },
    ...SHARED_CELL_MARKUP,
  ];
  cell.position(position.x, position.y).resize(width, height);
  cell.attr({
    body: { fill: "#ffffff", stroke: "#adb5bd", strokeWidth: 2, rx: 5, ry: 5 },
    ...SHARED_CELL_ATTRS,
  });
  cell.set({ type: shapeType, ...SHARED_CELL_DEFAULTS });
  _attachPortsToCell(cell, []);
  return cell;
}

function _buildEllipseCell(position, namespace, shapeType, width, height) {
  const cell = new namespace.standard.Ellipse();
  cell.markup = [
    { tagName: "ellipse", selector: "body", className: "body" },
    ...SHARED_CELL_MARKUP,
  ];
  cell.position(position.x, position.y).resize(width, height);
  cell.attr({
    body: { fill: "#ffffff", stroke: "#adb5bd", strokeWidth: 2 },
    ...SHARED_CELL_ATTRS,
  });
  cell.set({ type: shapeType, ...SHARED_CELL_DEFAULTS });
  _attachPortsToCell(cell, []);
  return cell;
}

function _buildPolygonCell(
  position,
  namespace,
  shapeType,
  width,
  height,
  points,
) {
  const cell = new namespace.standard.Polygon();
  cell.markup = [
    { tagName: "polygon", selector: "body", className: "body" },
    ...SHARED_CELL_MARKUP,
  ];
  cell.position(position.x, position.y).resize(width, height);
  cell.attr({
    body: {
      refPoints: points.map((p) => p.join(",")).join(" "),
      fill: "#ffffff",
      stroke: "#adb5bd",
      strokeWidth: 2,
    },
    ...SHARED_CELL_ATTRS,
  });
  cell.set({ type: shapeType, ...SHARED_CELL_DEFAULTS });
  _attachPortsToCell(cell, points);
  return cell;
}

// =============================================================
// DiagramEditor — the main controller
// =============================================================

class DiagramEditor extends EventBus {
  constructor(container) {
    super();
    this.container = container;
    this.gridSize = 10;
    this.clearanceUnits = 4;
    this.nodeMap = new Map(); // jointCell.id → Node
    this.edgeMap = new Map(); // jointLink.id → Edge
    this.selection = null;
    this.autoPortsOn = true;
    this.isLoading = false;
    this.touchState = null;
    this.pointerDownAt = null;
    this.selectionWasAlreadyActive = false;
    this.registeredNodeTypes = {};

    this._buildLayout();
    this._setupRenderer();
    this._attachButtonListeners();
    this._attachDiagramListeners();
    this._attachTouchListeners();
    this._attachKeyboardShortcuts();

    // any mutation to the diagram bubbles up as a single 'change' event,
    // but only when we're not in the middle of a bulk deserialize
    [
      "node:add",
      "node:remove",
      "node:change",
      "node:move",
      "edge:add",
      "edge:remove",
      "edge:change",
    ].forEach((event) => {
      this.on(event, () => {
        if (!this.isLoading) {
          this.emit("change");
        }
      });
    });

    // collapse both sidebars by default on mobile to save space
    if (this._isMobile()) {
      this._setSidebarCollapsed(this.leftSidebar, true);
      this._setSidebarCollapsed(this.rightSidebar, true);
    }
  }

  /**
   * Public: register a custom node type in the shape library
   */
  registerNodeType(label, NodeClass) {
    const item = this.shapeLibrary.appendChild(
      this._makeElement("div", "wf-node-template"),
    );
    item.textContent = label;
    item.draggable = true;
    item.dataset.nodeTypeLabel = label;

    // on mobile, tapping a library item adds the node directly
    item.addEventListener("click", () => {
      if (!this._isMobile()) {
        return;
      }
      this.addNode(new NodeClass());
      if (!this.leftSidebar.classList.contains("wf-collapsed")) {
        this._toggleSidebar(this.leftSidebar);
      }
    });

    item.addEventListener("dragstart", (event) =>
      event.dataTransfer.setData("customNode", label),
    );
    this.registeredNodeTypes[label] = NodeClass;
  }

  /**
   * Public: add a node to the diagram
   */
  addNode(node, canvasX, canvasY) {
    const namespace = joint.shapes;
    const area = this.canvasArea;

    // default to the centre of the visible canvas area
    const centerX = canvasX ?? area.clientWidth / 2;
    const centerY = canvasY ?? area.clientHeight / 2;

    // convert screen coordinates to diagram coordinates
    const localPosition = this.renderer.clientToLocalPoint({
      x: centerX + area.getBoundingClientRect().left,
      y: centerY + area.getBoundingClientRect().top,
    });

    // squarish shapes (Square, Circle, Diamond) use equal dimensions
    const isSquarish =
      node instanceof Square ||
      node instanceof Circle ||
      node instanceof Diamond;
    const width = isSquarish ? 80 : 140;
    const height = isSquarish ? 80 : 50;

    // find a spot that doesn't overlap existing nodes
    const openPosition = this._findOpenPosition(
      localPosition.x - width / 2,
      localPosition.y - height / 2,
      width,
      height,
    );

    // build the underlying JointJS cell and wire it to our Node object
    const cell = node._buildCell(openPosition, namespace);
    cell.attr("label/text", node._label);
    node.cell = cell;
    node.editor = this;

    // apply any built-in property overrides passed in via the constructor
    const builtIns = [
      "label",
      "labelColor",
      "labelFontSize",
      "description",
      "descriptionColor",
      "backgroundColor",
      "borderColor",
      "borderWidth",
      "imageUrl",
      "imageWidth",
      "imageHeight",
      "status",
      "priority",
    ];
    const initOptions = node._initOptions || {};
    builtIns.forEach((key) => {
      const value = initOptions[key] ?? node[`_init_${key}`];
      if (value !== undefined) {
        node[key] = value;
      }
    });

    // persist custom property values onto the cell model
    Object.entries(node.customProps).forEach(([key, value]) =>
      cell.set(`custom_${key}`, value),
    );

    cell.addTo(this.graph);
    this.nodeMap.set(cell.id, node);
    this.clearSelection();

    // auto-resize and run the render function once the cell is in the DOM
    const ready = (async () => {
      await this._waitForRender(cell);
      await this._resizeNodeAsync(cell);

      if (node.renderFn) {
        node.renderFn(node);
      }

      await this._waitForRender(cell);

      node.on("change", (changedNode) => this.emit("node:change", changedNode));
      node.on("move", (movedNode) => this.emit("node:move", movedNode));

      return node;
    })();

    this.emit("node:add", node);
    return ready;
  }

  removeNode(node) {
    node.remove();
    this.nodeMap.delete(node.id);
    this.emit("node:remove", node);
  }

  getNodes() {
    return [...this.nodeMap.values()];
  }

  getEdges() {
    return [...this.edgeMap.values()];
  }

  // viewport controls

  panTo(x, y) {
    this.renderer.translate(x, y);
    return this;
  }

  centerContent() {
    const bbox = this.graph.getBBox();
    if (!bbox) {
      return this;
    }
    const size = this.renderer.getComputedSize();
    this.renderer.scale(1, 1);
    this.renderer.translate(
      size.width / 2 - (bbox.x + bbox.width / 2),
      size.height / 2 - (bbox.y + bbox.height / 2),
    );
    return this;
  }

  zoomIn(factor = 1.25) {
    this._zoomAtCenter(factor);
    return this;
  }

  zoomOut(factor = 0.8) {
    this._zoomAtCenter(factor);
    return this;
  }

  zoomReset() {
    this._zoomAtCenter(1 / this.renderer.scale().sx);
    return this;
  }

  zoomToFit() {
    this.renderer.scaleContentToFit({
      padding: 50,
      minScale: 0.2,
      maxScale: 2,
    });
    return this;
  }

  getZoomLevel() {
    return this.renderer.scale().sx;
  }

  getSelectedItem() {
    return this.selection;
  }

  clearSelection() {
    this._deselectAll();
    return this;
  }

  setAutoPortSwitching(enabled) {
    this.autoPortsOn = enabled;
    this.autoPortToggleButton.classList.toggle("active", enabled);

    if (enabled) {
      this.graph
        .getElements()
        .forEach((element) => this._updateConnectionPorts(element));
    }
    return this;
  }

  autoArrange() {
    // build a dagre graph from the current nodes and edges
    const dagreGraph = new dagre.graphlib.Graph();
    dagreGraph.setGraph({
      rankdir: "TB",
      ranksep: 60,
      nodesep: 40,
      marginx: 40,
      marginy: 40,
    });
    dagreGraph.setDefaultEdgeLabel(() => ({}));

    this.graph.getElements().forEach((element) => {
      const { width, height } = element.size();
      dagreGraph.setNode(element.id, { width, height });
    });

    this.graph.getLinks().forEach((link) => {
      const source = link.getSourceElement();
      const target = link.getTargetElement();
      if (source && target) {
        dagreGraph.setEdge(source.id, target.id);
      }
    });

    // run the layout algorithm, then reposition each node
    dagre.layout(dagreGraph);

    this.graph.getElements().forEach((element) => {
      const layoutNode = dagreGraph.node(element.id);
      if (!layoutNode) {
        return;
      }
      element.position(
        Math.round(layoutNode.x - layoutNode.width / 2),
        Math.round(layoutNode.y - layoutNode.height / 2),
      );
    });

    if (this.autoPortsOn) {
      this.graph
        .getElements()
        .forEach((element) => this._updateConnectionPorts(element));
    }

    this.centerContent();
    return this;
  }

  /**
   * Public: serialize the diagram to a JSON string
   */
  serialize() {
    const nodes = [...this.nodeMap.values()].map((node) => ({
      id: node.cell.id,
      nodeClass: node.constructor.name,
      x: node.x,
      y: node.y,
      props: {
        label: node.label,
        labelColor: node.labelColor,
        labelFontSize: node.labelFontSize,
        description: node.description,
        descriptionColor: node.descriptionColor,
        backgroundColor: node.backgroundColor,
        borderColor: node.borderColor,
        borderWidth: node.borderWidth,
        imageUrl: node.imageUrl,
        imageWidth: node.imageWidth,
        imageHeight: node.imageHeight,
      },
      customProps: { ...node.customProps },
    }));

    const edges = [...this.edgeMap.values()].map((edge) => {
      // resolve a port object back to a 1-based index
      const resolvePort = (endpoint, node) => {
        const portId = endpoint?.port;
        if (!portId) {
          return null;
        }
        const index = node.cell
          .getPorts()
          .findIndex((port) => port.id === portId);
        return index >= 0 ? index + 1 : null;
      };

      return {
        sourceId: edge.source.cell.id,
        targetId: edge.target.cell.id,
        sourcePort: resolvePort(edge.link.source(), edge.source),
        targetPort: resolvePort(edge.link.target(), edge.target),
        label: edge.label,
        labelColor: edge.labelColor,
        labelFontSize: edge.labelFontSize,
        lineColor: edge.lineColor,
        lineWidth: edge.lineWidth,
        lineStyle: edge.lineStyle,
        sourceArrow: edge.sourceArrow,
        targetArrow: edge.targetArrow,
        connectorType: edge.connectorType,
        description: edge.description,
        vertices: edge.link.vertices(),
      };
    });

    return JSON.stringify({ nodes, edges });
  }

  /**
   * Public: deserialize a JSON string and rebuild the diagram
   */
  async deserialize(json) {
    const { nodes: nodeDataList, edges: edgeDataList } =
      typeof json === "string" ? JSON.parse(json) : json;
    if (!Array.isArray(nodeDataList)) {
      throw new Error("Invalid diagram file format.");
    }

    // clear the current diagram
    this._deselectAll();
    this.nodeMap.clear();
    this.edgeMap.clear();
    this.graph.clear();

    const nodeClassMap = {
      Rectangle,
      Square,
      Ellipse,
      Circle,
      Diamond,
      Triangle,
      Hexagon,
      Pentagon,
      Octagon,
      ...Object.fromEntries(
        Object.entries(this.registeredNodeTypes).map(([, cls]) => [
          cls.name,
          cls,
        ]),
      ),
    };

    // disable auto-ports and change events during the bulk load
    this.autoPortsOn = false;
    this.isLoading = true;

    // keep a map of old serialized IDs → new Node instances so edges can reconnect
    const oldIdToNode = {};

    for (const nodeData of nodeDataList) {
      const NodeClass = nodeClassMap[nodeData.nodeClass];
      if (!NodeClass) {
        console.warn(`Unknown node class: ${nodeData.nodeClass}`);
        continue;
      }

      // reconstruct the node and its cell
      const node = new NodeClass({
        ...nodeData.props,
        ...nodeData.customProps,
      });
      const cell = node._buildCell(
        { x: nodeData.x, y: nodeData.y },
        joint.shapes,
      );
      cell.attr("label/text", node._label);
      node.cell = cell;
      node.editor = this;

      // re-apply built-in property overrides
      const builtIns = [
        "label",
        "labelColor",
        "labelFontSize",
        "description",
        "descriptionColor",
        "backgroundColor",
        "borderColor",
        "borderWidth",
        "imageUrl",
        "imageWidth",
        "imageHeight",
        "status",
        "priority",
      ];
      const initOptions = node._initOptions || {};
      builtIns.forEach((key) => {
        const value = initOptions[key] ?? node[`_init_${key}`];
        if (value !== undefined) {
          node[key] = value;
        }
      });

      Object.entries(node.customProps).forEach(([key, value]) =>
        cell.set(`custom_${key}`, value),
      );

      cell.addTo(this.graph);
      this.nodeMap.set(cell.id, node);

      // wait two frames so the DOM has rendered the cell before we measure it
      await new Promise((resolve) =>
        requestAnimationFrame(() => requestAnimationFrame(resolve)),
      );
      this._fitNodeToContent(cell);
      await new Promise((resolve) =>
        requestAnimationFrame(() => requestAnimationFrame(resolve)),
      );
      this.renderer.updateViews();

      if (node.renderFn) {
        node.renderFn(node);
      }

      node.on("change", (changedNode) => this.emit("node:change", changedNode));
      node.on("move", (movedNode) => this.emit("node:move", movedNode));

      oldIdToNode[nodeData.id] = node;
    }

    // reconnect edges using the old→new ID map
    for (const edgeData of edgeDataList) {
      const sourceNode = oldIdToNode[edgeData.sourceId];
      const targetNode = oldIdToNode[edgeData.targetId];
      if (!sourceNode || !targetNode) {
        continue;
      }

      const edge = sourceNode.connect(
        targetNode,
        edgeData.sourcePort,
        edgeData.targetPort,
      );
      if (!edge) {
        continue;
      }

      if (edgeData.label) {
        edge.label = edgeData.label;
      }
      if (edgeData.labelColor) {
        edge.labelColor = edgeData.labelColor;
      }
      if (edgeData.labelFontSize) {
        edge.labelFontSize = edgeData.labelFontSize;
      }

      edge.lineColor = edgeData.lineColor;
      edge.lineWidth = edgeData.lineWidth;
      edge.lineStyle = edgeData.lineStyle;
      edge.sourceArrow = edgeData.sourceArrow;
      edge.targetArrow = edgeData.targetArrow;
      edge.connectorType = edgeData.connectorType;

      if (edgeData.description) {
        edge.description = edgeData.description;
      }
      if (edgeData.vertices?.length) {
        edge.link.vertices(edgeData.vertices);
      }
    }

    this.isLoading = false;

    // force a full re-render, then restore pinned port indices
    const allCells = this.graph.getCells();
    this.graph.resetCells(allCells);
    await new Promise((resolve) =>
      requestAnimationFrame(() => requestAnimationFrame(resolve)),
    );

    this.graph.getLinks().forEach((link) => {
      const edge = this.edgeMap.get(link.id);
      if (!edge) {
        return;
      }

      const savedEdge = edgeDataList.find(
        (data) =>
          oldIdToNode[data.sourceId]?.cell.id === edge.source.cell.id &&
          oldIdToNode[data.targetId]?.cell.id === edge.target.cell.id,
      );
      if (!savedEdge) {
        return;
      }

      if (savedEdge.sourcePort != null) {
        link.set("sourcePort", savedEdge.sourcePort);
      }
      if (savedEdge.targetPort != null) {
        link.set("targetPort", savedEdge.targetPort);
      }
    });

    this.emit("change");
    return this;
  }

  /**
   * Internal: hide/show mobile floating buttons
   */
  _updateMobileButtonVisibility() {
    if (!this._isMobile()) {
      return;
    }

    // hide the floating buttons whenever a sidebar is open or an item is selected
    const shouldHide =
      this.selection !== null ||
      !this.leftSidebar.classList.contains("wf-collapsed") ||
      !this.rightSidebar.classList.contains("wf-collapsed");

    const display = shouldHide ? "none" : "";
    this.topRightButtons.style.display = display;
    this.topLeftButtons.style.display = display;
    this.bottomLeftButtons.style.display = display;
  }

  /**
   * Internal: build the full DOM layout
   */
  _buildLayout() {
    this.container.innerHTML = "";
    this.container.style.cssText =
      "display:flex; height:100%; overflow:hidden;";

    // left sidebar — shape library
    this.leftSidebar = this.container.appendChild(
      this._makeElement("div", "wf-sidebar wf-sidebar-left"),
    );
    const leftHeader = this.leftSidebar.appendChild(
      this._makeElement("div", "wf-sidebar-header"),
    );
    leftHeader.innerHTML = `
      <span class="wf-sidebar-title">Library</span>
      <button class="wf-collapse-btn" title="Toggle Library">
        <svg class="wf-collapse-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg>
      </button>
      <span class="wf-collapsed-label" style="display:none">Library</span>`;
    this.leftCollapseButton = leftHeader.querySelector(".wf-collapse-btn");
    this.leftCollapseIcon = leftHeader.querySelector(".wf-collapse-icon");
    this.shapeLibrary = this.leftSidebar.appendChild(
      this._makeElement("div", "wf-library wf-sidebar-body"),
    );
    this._buildShapeLibrary();

    // canvas area
    this.canvasArea = this.container.appendChild(
      this._makeElement("div", "wf-canvas-container"),
    );
    this.paperElement = this.canvasArea.appendChild(
      this._makeElement("div", "wf-paper"),
    );

    // top-right buttons (export / import)
    this.topRightButtons = this.canvasArea.appendChild(
      this._makeElement("div"),
    );
    this.topRightButtons.style.cssText =
      "position:absolute; top:20px; right:20px; z-index:100; display:flex; gap:8px;";

    this.exportButton = this.topRightButtons.appendChild(
      this._makeElement("button", "wf-ctrl-btn"),
    );
    this.exportButton.title = "Export";
    this.exportButton.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>`;

    this.importButton = this.topRightButtons.appendChild(
      this._makeElement("button", "wf-ctrl-btn"),
    );
    this.importButton.title = "Import";
    this.importButton.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>`;

    // hidden file input used by the import button
    this.importFileInput = this.canvasArea.appendChild(
      this._makeElement("input"),
    );
    this.importFileInput.type = "file";
    this.importFileInput.accept = ".json";
    this.importFileInput.style.display = "none";

    // top-left buttons (auto-port toggle / auto-arrange)
    this.topLeftButtons = this.canvasArea.appendChild(this._makeElement("div"));
    this.topLeftButtons.style.cssText =
      "position:absolute; top:20px; left:20px; z-index:100; display:flex; gap:8px;";

    this.autoPortToggleButton = this.topLeftButtons.appendChild(
      this._makeElement("button", "wf-ctrl-btn active"),
    );
    this.autoPortToggleButton.title = "Toggle Auto Port Switching";
    this.autoPortToggleButton.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 7h-9m3 10H5M16 12H8M7 7l-4 4 4 4M17 9l4 4-4 4"/></svg>`;

    this.autoArrangeButton = this.topLeftButtons.appendChild(
      this._makeElement("button", "wf-ctrl-btn"),
    );
    this.autoArrangeButton.title = "Auto Arrange";
    this.autoArrangeButton.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="4" rx="1"/><rect x="3" y="10" width="7" height="4" rx="1"/><rect x="3" y="17" width="7" height="4" rx="1"/><rect x="14" y="5" width="7" height="4" rx="1"/><rect x="14" y="15" width="7" height="4" rx="1"/><line x1="10" y1="5" x2="14" y2="7"/><line x1="10" y1="12" x2="14" y2="7"/><line x1="10" y1="12" x2="14" y2="17"/><line x1="10" y1="19" x2="14" y2="17"/></svg>`;

    // bottom-left buttons (zoom controls)
    this.bottomLeftButtons = this.canvasArea.appendChild(
      this._makeElement("div", "wf-bottom-left"),
    );
    const zoomGroup = this.bottomLeftButtons.appendChild(
      this._makeElement("div", "wf-zoom-group"),
    );
    this.zoomToFitButton = this._makeZoomButton(
      zoomGroup,
      "Fit",
      `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 3h6v6M9 21H3v-6M21 3l-7 7M3 21l7-7"/></svg>`,
    );
    this.zoomResetButton = this._makeZoomButton(zoomGroup, "Reset Zoom", "1:1");
    this.zoomInButton = this._makeZoomButton(zoomGroup, "Zoom In", "+");
    this.zoomOutButton = this._makeZoomButton(zoomGroup, "Zoom Out", "−");
    this.centerButton = this._makeZoomButton(
      zoomGroup,
      "Center",
      `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg>`,
    );

    // zoom buttons don't make sense on touch devices
    if (this._isMobile()) {
      this.zoomInButton.style.display = "none";
      this.zoomOutButton.style.display = "none";
    }

    // right-click context menu
    this.contextMenu = this.canvasArea.appendChild(
      this._makeElement("div", "wf-context-menu"),
    );
    this.contextMenu.innerHTML = `
      <div class="wf-menu-item" data-action="duplicate">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
        Duplicate
      </div>
      <div class="wf-menu-item danger" data-action="delete">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
        Delete
      </div>`;

    // right sidebar — properties panel
    this.rightSidebar = this.container.appendChild(
      this._makeElement("div", "wf-sidebar wf-sidebar-right"),
    );
    const rightHeader = this.rightSidebar.appendChild(
      this._makeElement("div", "wf-sidebar-header"),
    );
    rightHeader.innerHTML = `
      <button class="wf-collapse-btn" title="Toggle Properties">
        <svg class="wf-collapse-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>
      </button>
      <span class="wf-sidebar-title">Properties</span>
      <span class="wf-collapsed-label" style="display:none">Properties</span>
      <div class="wf-header-actions wf-sidebar-body" style="display:none;">
        <button class="wf-header-btn" data-action="focus" title="Center on Element">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/></svg>
        </button>
        <button class="wf-header-btn" data-action="duplicate" title="Duplicate">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
        </button>
        <button class="wf-header-btn danger" data-action="delete" title="Delete">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
        </button>
      </div>`;
    this.rightCollapseButton = rightHeader.querySelector(".wf-collapse-btn");
    this.rightCollapseIcon = rightHeader.querySelector(".wf-collapse-icon");
    this.propertiesHeaderActions =
      rightHeader.querySelector(".wf-header-actions");

    this.nodePropertiesPanel = this._buildNodePropertiesPanel();
    this.edgePropertiesPanel = this._buildEdgePropertiesPanel();

    this.noSelectionMessage = this.rightSidebar.appendChild(
      this._makeElement("div", "wf-no-selection wf-sidebar-body"),
    );
    this.noSelectionMessage.textContent = "Select an item to view properties";

    this._showPropertiesPanel("none");
  }

  /**
   * Internal: populate the shape library panel with built-in shapes
   */
  _buildShapeLibrary() {
    const builtInShapes = [
      { label: "Rectangle", type: "rect", cls: Rectangle },
      { label: "Square", type: "square", cls: Square },
      { label: "Ellipse", type: "ellipse", cls: Ellipse },
      { label: "Circle", type: "circle", cls: Circle },
      { label: "Diamond", type: "diamond", cls: Diamond },
      { label: "Triangle", type: "triangle", cls: Triangle },
      { label: "Hexagon", type: "hexagon", cls: Hexagon },
      { label: "Pentagon", type: "pentagon", cls: Pentagon },
      { label: "Octagon", type: "octagon", cls: Octagon },
    ];

    builtInShapes.forEach(({ label, type, cls }) => {
      const item = this.shapeLibrary.appendChild(
        this._makeElement("div", "wf-node-template"),
      );
      item.textContent = label;
      item.draggable = true;
      item.dataset.type = type;

      item.addEventListener("dragstart", (event) =>
        event.dataTransfer.setData("type", type),
      );

      // on mobile, tapping the item adds it to the canvas
      item.addEventListener("click", () => {
        if (!this._isMobile()) {
          return;
        }
        this.addNode(new cls(label.toUpperCase()));
        if (!this.leftSidebar.classList.contains("wf-collapsed")) {
          this._toggleSidebar(this.leftSidebar);
        }
      });
    });
  }

  /**
   * Internal: build the node properties form
   */
  _buildNodePropertiesPanel() {
    const panel = this.rightSidebar.appendChild(
      this._makeElement(
        "div",
        "wf-properties wf-prop-panel-element wf-sidebar-body",
      ),
    );
    panel.style.display = "none";
    panel.innerHTML = `
      <div class="wf-prop-group">
        <label>Label</label>
        <input type="text" data-prop="label">
      </div>
      <div class="wf-prop-group">
        <label>Font Size (%)</label>
        <input type="number" data-prop="labelFontSize" min="10" max="500">
      </div>
      <div class="wf-prop-group">
        <label>Background Color</label>
        <div class="wf-color-row">
          <input type="color" data-prop="backgroundColor" data-pair="backgroundColorHex">
          <input type="text"  data-prop="backgroundColorHex" data-pair-picker="backgroundColor" placeholder="#FFFFFF">
        </div>
      </div>
      <div class="wf-prop-group">
        <label>Border Color</label>
        <div class="wf-color-row">
          <input type="color" data-prop="borderColor" data-pair="borderColorHex">
          <input type="text"  data-prop="borderColorHex" data-pair-picker="borderColor" placeholder="#ADB5BD">
        </div>
      </div>
      <div class="wf-prop-group">
        <label>Label Color</label>
        <div class="wf-color-row">
          <input type="color" data-prop="labelColor" data-pair="labelColorHex">
          <input type="text"  data-prop="labelColorHex" data-pair-picker="labelColor" placeholder="#212529">
        </div>
      </div>
      <div class="wf-prop-group">
        <label>Description Color</label>
        <div class="wf-color-row">
          <input type="color" data-prop="descriptionColor" data-pair="descriptionColorHex">
          <input type="text"  data-prop="descriptionColorHex" data-pair-picker="descriptionColor" placeholder="#6C757D">
        </div>
      </div>
      <div class="wf-prop-group">
        <label>Description</label>
        <textarea data-prop="description"></textarea>
      </div>
      <div class="wf-prop-group">
        <label>Image URL</label>
        <div class="wf-image-row">
          <input type="text" data-prop="imageUrl" placeholder="https://...">
          <button class="wf-upload-btn" data-action="upload-image" title="Upload Image">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
          </button>
          <input type="file" data-action="image-file" style="display:none;" accept="image/*">
        </div>
      </div>
      <div class="wf-prop-row">
        <div class="wf-prop-group">
          <label>Image Width</label>
          <input type="number" data-prop="imageWidth" min="0">
        </div>
        <div class="wf-prop-group">
          <label>Image Height</label>
          <input type="number" data-prop="imageHeight" min="0">
        </div>
      </div>`;
    return panel;
  }

  /**
   * Internal: build the edge properties form
   */
  _buildEdgePropertiesPanel() {
    const panel = this.rightSidebar.appendChild(
      this._makeElement(
        "div",
        "wf-properties wf-prop-panel-link wf-sidebar-body",
      ),
    );
    panel.style.display = "none";
    panel.innerHTML = `
      <div class="wf-prop-group">
        <label>Label</label>
        <input type="text" data-prop="label">
      </div>
      <div class="wf-prop-group">
        <label>Label Color</label>
        <div class="wf-color-row">
          <input type="color" data-prop="labelColor" data-pair="labelColorHex">
          <input type="text"  data-prop="labelColorHex" data-pair-picker="labelColor" placeholder="#333333">
        </div>
      </div>
      <div class="wf-prop-group">
        <label>Font Size (%)</label>
        <input type="number" data-prop="labelFontSize" min="10" max="500">
      </div>
      <div class="wf-prop-group">
        <label>Connector Type</label>
        <select data-prop="connectorType">
          <option value="elbow">Elbow (Default)</option>
          <option value="straight">Straight</option>
          <option value="curved">Curved</option>
        </select>
      </div>
      <div class="wf-prop-group">
        <label>Line Color</label>
        <div class="wf-color-row">
          <input type="color" data-prop="lineColor" data-pair="lineColorHex">
          <input type="text"  data-prop="lineColorHex" data-pair-picker="lineColor" placeholder="#495057">
        </div>
      </div>
      <div class="wf-prop-group">
        <label>Line Style</label>
        <select data-prop="lineStyle">
          <option value="solid">Solid</option>
          <option value="dashed">Dashed</option>
          <option value="dotted">Dotted</option>
        </select>
      </div>
      <div class="wf-prop-group">
        <label>Source Arrow</label>
        <select data-prop="sourceArrow">
          <option value="none">None</option>
          <option value="classic">Classic</option>
          <option value="block">Block</option>
        </select>
      </div>
      <div class="wf-prop-group">
        <label>Target Arrow</label>
        <select data-prop="targetArrow">
          <option value="none">None</option>
          <option value="classic">Classic</option>
          <option value="block">Block</option>
        </select>
      </div>
      <div class="wf-prop-group">
        <label>Description</label>
        <textarea data-prop="description"></textarea>
      </div>`;
    return panel;
  }

  /**
   * Internal: set up the JointJS graph and renderer
   */
  _setupRenderer() {
    this.graph = new joint.dia.Graph({}, { cellNamespace: joint.shapes });
    this.renderer = new joint.dia.Paper({
      el: this.paperElement,
      model: this.graph,
      width: "100%",
      height: "100%",
      gridSize: this.gridSize,
      drawGrid: { name: "dot", color: "#e9ecef" },
      background: { color: "#ffffff" },
      cellNamespace: joint.shapes,
      defaultConnector: { name: "rounded" },
      defaultRouter: {
        name: "manhattan",
        args: { step: this.gridSize, padding: 30, maxIter: 200 },
      },
      interactive: {
        linkMove: true,
        vertexAdd: true,
        vertexMove: true,
        elementMove: true,
      },
      preventDefaultBlankAction: false,
      linkPinning: false,
      snapLinks: { radius: 25 },
      markAvailable: true,
      defaultLink: () =>
        new joint.shapes.standard.Link({
          attrs: {
            line: {
              stroke: "#495057",
              strokeWidth: 2,
              targetMarker: ARROW_MARKERS.classic,
            },
          },
        }),
      validateConnection: (
        sourceView,
        sourceMagnet,
        targetView,
        targetMagnet,
      ) => {
        // prevent self-connections and connections from non-port elements
        return sourceMagnet && targetMagnet && sourceView !== targetView;
      },
    });
  }

  /**
   * Internal: wire up all button and UI event listeners
   */
  _attachButtonListeners() {
    // export
    this.exportButton.addEventListener("click", () => {
      try {
        const blob = new Blob([this.serialize()], { type: "application/json" });
        const url = URL.createObjectURL(blob);
        const anchor = Object.assign(document.createElement("a"), {
          href: url,
          download: "diagram.json",
        });

        document.body.appendChild(anchor);
        anchor.click();
        document.body.removeChild(anchor);
        setTimeout(() => URL.revokeObjectURL(url), 1000);
      } catch (error) {
        alert("Export failed: " + error.message);
      }
    });

    // import
    this.importButton.addEventListener("click", () =>
      this.importFileInput.click(),
    );

    this.importFileInput.addEventListener("change", (event) => {
      const file = event.target.files[0];
      if (!file) {
        return;
      }
      const reader = new FileReader();
      reader.onload = (loadEvent) =>
        this.deserialize(loadEvent.target.result).catch((error) =>
          alert("Import failed: " + error.message),
        );
      reader.readAsText(file);
      event.target.value = "";
    });

    // sidebar toggles
    this.leftCollapseButton.addEventListener("click", () =>
      this._toggleSidebar(this.leftSidebar),
    );
    this.rightCollapseButton.addEventListener("click", () =>
      this._toggleSidebar(this.rightSidebar),
    );

    // toolbar buttons
    this.autoPortToggleButton.addEventListener("click", () =>
      this.setAutoPortSwitching(!this.autoPortsOn),
    );
    this.autoArrangeButton.addEventListener("click", () => this.autoArrange());

    // zoom buttons
    this.zoomToFitButton.addEventListener("click", () => this.zoomToFit());
    this.zoomResetButton.addEventListener("click", () => this.zoomReset());
    this.zoomInButton.addEventListener("click", () => this.zoomIn());
    this.zoomOutButton.addEventListener("click", () => this.zoomOut());
    this.centerButton.addEventListener("click", () => this.centerContent());

    // mouse wheel zoom / pan
    this.renderer.el.addEventListener(
      "wheel",
      (event) => {
        event.preventDefault();
        if (event.ctrlKey) {
          // ctrl + scroll = zoom centred on the cursor
          this._zoomAtPoint(
            Math.exp(-event.deltaY * 0.0015),
            event.clientX,
            event.clientY,
          );
        } else {
          // plain scroll = pan
          const translation = this.renderer.translate();
          this.renderer.translate(
            translation.tx - event.deltaX,
            translation.ty - event.deltaY,
          );
        }
      },
      { passive: false },
    );

    // right-click context menu
    this.contextMenu.addEventListener("click", (event) => {
      const action = event.target.closest("[data-action]")?.dataset.action;
      if (action === "duplicate") {
        this._duplicateSelected();
      }
      if (action === "delete") {
        this._deleteSelected();
      }
      this.contextMenu.style.display = "none";
    });

    window.addEventListener("click", (event) => {
      if (!event.target.closest(".wf-context-menu")) {
        this.contextMenu.style.display = "none";
      }
    });

    // properties panel header actions (focus, duplicate, delete)
    this.propertiesHeaderActions.addEventListener("click", (event) => {
      const action = event.target.closest("[data-action]")?.dataset.action;
      if (!action) {
        return;
      }
      if (action === "focus") {
        this._focusCameraOnSelection();
      }
      if (action === "duplicate") {
        this._duplicateSelected();
        if (this._isMobile()) {
          this._toggleSidebar(this.rightSidebar);
        }
      }
      if (action === "delete") {
        this._deleteSelected();
        if (this._isMobile()) {
          this._toggleSidebar(this.rightSidebar);
        }
      }
    });

    // property input changes
    this.nodePropertiesPanel.addEventListener("input", (event) =>
      this._handleNodePropertyChange(event),
    );
    this.nodePropertiesPanel.addEventListener("change", (event) =>
      this._handleNodePropertyChange(event),
    );
    this.edgePropertiesPanel.addEventListener("input", (event) =>
      this._handleEdgePropertyChange(event),
    );
    this.edgePropertiesPanel.addEventListener("change", (event) =>
      this._handleEdgePropertyChange(event),
    );

    // drag-and-drop from the shape library
    this.canvasArea.addEventListener("dragover", (event) =>
      event.preventDefault(),
    );
    this.canvasArea.addEventListener("drop", (event) => {
      event.preventDefault();

      // check if the dragged item is a custom node type
      const customLabel = event.dataTransfer.getData("customNode");
      if (customLabel && this.registeredNodeTypes?.[customLabel]) {
        const NodeClass = this.registeredNodeTypes[customLabel];
        const dropPosition = this.renderer.clientToLocalPoint({
          x: event.clientX,
          y: event.clientY,
        });

        const node = new NodeClass();
        const openPosition = this._findOpenPosition(
          dropPosition.x - 70,
          dropPosition.y - 25,
          140,
          50,
        );
        const cell = node._buildCell(openPosition, joint.shapes);
        cell.attr("label/text", node._label);
        node.cell = cell;
        node.editor = this;

        Object.entries(node.customProps).forEach(([key, value]) =>
          cell.set(`custom_${key}`, value),
        );
        cell.addTo(this.graph);
        this.nodeMap.set(cell.id, node);
        this.clearSelection();

        (async () => {
          await this._waitForRender(cell);
          await this._resizeNodeAsync(cell);

          if (node.renderFn) {
            node.renderFn(node);
          }

          await this._waitForRender(cell);
          node.on("change", (changedNode) =>
            this.emit("node:change", changedNode),
          );
          node.on("move", (movedNode) => this.emit("node:move", movedNode));
          this._selectItem(node);
        })();

        this.emit("node:add", node);
        return;
      }

      // otherwise it must be a built-in shape type
      const droppedType = event.dataTransfer.getData("type");
      const builtInShapes = [
        { type: "rect", cls: Rectangle },
        { type: "square", cls: Square },
        { type: "ellipse", cls: Ellipse },
        { type: "circle", cls: Circle },
        { type: "diamond", cls: Diamond },
        { type: "triangle", cls: Triangle },
        { type: "hexagon", cls: Hexagon },
        { type: "pentagon", cls: Pentagon },
        { type: "octagon", cls: Octagon },
      ];

      const match = builtInShapes.find((shape) => shape.type === droppedType);
      if (!match) {
        return;
      }

      const label = droppedType.charAt(0).toUpperCase() + droppedType.slice(1);
      const dropPosition = this.renderer.clientToLocalPoint({
        x: event.clientX,
        y: event.clientY,
      });

      const node = new match.cls(label.toUpperCase());
      const isSquarish = ["square", "circle", "diamond"].includes(droppedType);
      const width = isSquarish ? 80 : 140;
      const height = isSquarish ? 80 : 50;

      const openPosition = this._findOpenPosition(
        dropPosition.x - width / 2,
        dropPosition.y - height / 2,
        width,
        height,
      );
      const cell = node._buildCell(openPosition, joint.shapes);
      cell.attr("label/text", node._label);
      node.cell = cell;
      node.editor = this;
      cell.addTo(this.graph);
      this.nodeMap.set(cell.id, node);
      this.clearSelection();

      (async () => {
        await this._waitForRender(cell);
        await this._resizeNodeAsync(cell);
        await this._waitForRender(cell);
        node.on("change", (changedNode) =>
          this.emit("node:change", changedNode),
        );
        node.on("move", (movedNode) => this.emit("node:move", movedNode));
        this._selectItem(node);
      })();

      this.emit("node:add", node);
    });

    // image upload in the node properties panel
    this.nodePropertiesPanel
      .querySelector('[data-action="upload-image"]')
      .addEventListener("click", () => {
        this.nodePropertiesPanel
          .querySelector('[data-action="image-file"]')
          .click();
      });

    this.nodePropertiesPanel
      .querySelector('[data-action="image-file"]')
      .addEventListener("change", (event) => {
        const file = event.target.files[0];
        if (!file) {
          return;
        }
        const reader = new FileReader();
        reader.onload = (loadEvent) => {
          this.nodePropertiesPanel.querySelector(
            '[data-prop="imageUrl"]',
          ).value = loadEvent.target.result;

          if (this.selection instanceof Node) {
            this.selection.imageUrl = loadEvent.target.result;
          }
        };
        reader.readAsDataURL(file);
      });
  }

  /**
   * Internal: handle a property input change in the node properties panel
   */
  _handleNodePropertyChange(event) {
    if (!(this.selection instanceof Node)) {
      return;
    }
    const input = event.target.closest("[data-prop]");
    if (!input) {
      return;
    }

    const prop = input.dataset.prop;
    const value =
      input.type === "number" ? parseFloat(input.value) : input.value;

    // keep the paired hex text input / colour picker in sync
    if (input.dataset.pair) {
      this.nodePropertiesPanel.querySelector(
        `[data-prop="${input.dataset.pair}"]`,
      ).value = value;
    }
    if (input.dataset.pairPicker) {
      this.nodePropertiesPanel.querySelector(
        `[data-prop="${input.dataset.pairPicker}"]`,
      ).value = value;
    }

    // strip the 'Hex' suffix so 'backgroundColorHex' maps to 'backgroundColor'
    const resolvedProp = prop.endsWith("Hex") ? prop.slice(0, -3) : prop;
    if (resolvedProp in this.selection) {
      this.selection[resolvedProp] = value;
    }
  }

  /**
   * Internal: handle a property input change in the edge properties panel
   */
  _handleEdgePropertyChange(event) {
    if (!(this.selection instanceof Edge)) {
      return;
    }
    const input = event.target.closest("[data-prop]");
    if (!input) {
      return;
    }

    const prop = input.dataset.prop;
    const value =
      input.type === "number" ? parseFloat(input.value) : input.value;

    if (input.dataset.pair) {
      this.edgePropertiesPanel.querySelector(
        `[data-prop="${input.dataset.pair}"]`,
      ).value = value;
    }
    if (input.dataset.pairPicker) {
      this.edgePropertiesPanel.querySelector(
        `[data-prop="${input.dataset.pairPicker}"]`,
      ).value = value;
    }

    const resolvedProp = prop.endsWith("Hex") ? prop.slice(0, -3) : prop;
    if (resolvedProp in this.selection) {
      this.selection[resolvedProp] = value;
    }
  }

  /**
   * Internal: wire up all JointJS diagram event listeners
   */
  _attachDiagramListeners() {
    // custom circular arrowhead used on the source end of selected edges
    const CustomSourceArrowhead = joint.linkTools.SourceArrowhead.extend({
      tagName: "circle",
      attributes: {
        r: 6,
        fill: "#3498db",
        stroke: "#fff",
        "stroke-width": 2,
        cursor: "move",
      },
    });

    // node pointer events

    this.renderer.on("element:pointerdown", (view, event) => {
      const node = this.nodeMap.get(view.model.id);
      if (!node) {
        return;
      }

      this.pointerDownAt = { x: event.clientX, y: event.clientY };
      this.selectionWasAlreadyActive = this.selection === node;
      this._deselectAll();

      if (!this._isMobile()) {
        node.cell.toFront();
      }
      this._selectItem(node);
    });

    this.renderer.on("element:pointerup", (view, event) => {
      const deltaX = Math.abs(
        event.clientX - (this.pointerDownAt?.x ?? event.clientX),
      );
      const deltaY = Math.abs(
        event.clientY - (this.pointerDownAt?.y ?? event.clientY),
      );

      // if the user tapped (not dragged) an already-selected node, open the properties panel
      if (
        deltaX < 5 &&
        deltaY < 5 &&
        this.selectionWasAlreadyActive &&
        this.rightSidebar.classList.contains("wf-collapsed")
      ) {
        this._toggleSidebar(this.rightSidebar);
      }

      this.pointerDownAt = null;
      this.selectionWasAlreadyActive = false;

      // snap the node to the nearest clear grid position after a drag
      if (!this._isMobile()) {
        const node = this.nodeMap.get(view.model.id);
        if (node) {
          const bbox = node.cell.getBBox();
          const openPosition = this._findOpenPosition(
            bbox.x,
            bbox.y,
            bbox.width,
            bbox.height,
            node.cell,
          );
          node.cell.position(openPosition.x, openPosition.y);
        }
      }
    });

    // edge pointer events

    this.renderer.on("link:pointerdown", (view) => {
      const edge = this.edgeMap.get(view.model.id);
      if (!edge) {
        return;
      }

      this._deselectAll();
      view.model.toFront();
      this._selectItem(edge);

      // attach editing tools to the selected edge
      view.addTools(
        new joint.dia.ToolsView({
          tools: [
            new joint.linkTools.Vertices(),
            new CustomSourceArrowhead(),
            new joint.linkTools.TargetArrowhead({ offset: -1 }),
            new joint.linkTools.Remove({ distance: 20 }),
          ],
        }),
      );
    });

    // show ports while dragging a new edge from a port
    this.renderer.on("cell:pointerdown tool:pointerdown", (view, event) => {
      if (event?.target?.closest?.(".joint-port")) {
        this.renderer.el.classList.add("wf-dragging-link");
      }
    });

    this.renderer.on("cell:pointerup link:pointerup tool:pointerup", () => {
      this.renderer.el.classList.remove("wf-dragging-link");
    });

    // context menu

    this.renderer.on("cell:contextmenu", (view, event) => {
      event.preventDefault();

      const rect = this.canvasArea.getBoundingClientRect();
      this.contextMenu.style.display = "block";
      this.contextMenu.style.left = event.clientX - rect.left + "px";
      this.contextMenu.style.top = event.clientY - rect.top + "px";

      // only show Duplicate for nodes, not edges
      this.contextMenu.querySelector(
        '[data-action="duplicate"]',
      ).style.display = this.nodeMap.has(view.model.id) ? "flex" : "none";
    });

    // blank canvas pan

    this.renderer.on("blank:pointerdown", (event) => {
      this._collapseAllSidebarsOnMobile();

      const startX = event.clientX;
      const startY = event.clientY;
      const initialTranslation = this.renderer.translate();

      if (this._isMobile()) {
        this._deselectAll();
        return;
      }

      const onMove = (moveEvent) =>
        this.renderer.translate(
          initialTranslation.tx + (moveEvent.clientX - startX),
          initialTranslation.ty + (moveEvent.clientY - startY),
        );

      const onUp = (upEvent) => {
        document.removeEventListener("mousemove", onMove);
        document.removeEventListener("mouseup", onUp);

        // a click (no drag) on blank space clears the selection
        if (
          Math.abs(upEvent.clientX - startX) < 5 &&
          Math.abs(upEvent.clientY - startY) < 5
        ) {
          this._deselectAll();
        }
      };

      document.addEventListener("mousemove", onMove);
      document.addEventListener("mouseup", onUp);
    });

    // edge connection completed

    this.renderer.on("link:connect", (view) => {
      const link = view.model;
      const sourceCell = link.getSourceElement();
      const targetCell = link.getTargetElement();
      if (!sourceCell || !targetCell) {
        return;
      }

      const sourceNode = this.nodeMap.get(sourceCell.id);
      const targetNode = this.nodeMap.get(targetCell.id);
      if (!sourceNode || !targetNode || this.edgeMap.has(link.id)) {
        return;
      }

      // disallow reverse duplicate edges (A→B when B→A already exists)
      const reverseExists = [...this.edgeMap.values()].some(
        (edge) => edge.source === targetNode && edge.target === sourceNode,
      );
      if (reverseExists) {
        link.remove();
        return;
      }

      const edge = new Edge(link, sourceNode, targetNode, this);
      this.edgeMap.set(link.id, edge);
      edge.on("change", (changedEdge) => this.emit("edge:change", changedEdge));
      this.emit("edge:add", edge);
    });

    // cell removal

    this.graph.on("remove", (cell) => {
      if (cell.isLink()) {
        const edge = this.edgeMap.get(cell.id);
        if (edge) {
          this.edgeMap.delete(cell.id);
          this.emit("edge:remove", edge);
        }
      } else {
        const node = this.nodeMap.get(cell.id);
        if (node) {
          this.nodeMap.delete(cell.id);
          this.emit("node:remove", node);
        }
      }
    });

    // update auto-ports whenever a node is moved
    this.graph.on("change:position", (cell) => {
      const node = this.nodeMap.get(cell.id);
      if (node) {
        this._updateConnectionPorts(cell);
        node.emit("move", node);
      }
    });
  }

  /**
   * Internal: wire up keyboard shortcuts
   */
  _attachKeyboardShortcuts() {
    this.clipboard = null;
    this.canvasArea.tabIndex = 0;
    this.canvasArea.style.outline = "none";
    this.canvasArea.focus();

    // reclaim focus whenever the user clicks the canvas (unless typing in a field)
    this.canvasArea.addEventListener("mousedown", () => {
      const tag = document.activeElement?.tagName;
      if (tag !== "INPUT" && tag !== "TEXTAREA") {
        this.canvasArea.focus();
      }
    });

    this.canvasArea.addEventListener("keydown", async (event) => {
      const tag = document.activeElement?.tagName;
      const isTyping =
        tag === "INPUT" || tag === "TEXTAREA" || tag === "SELECT";

      // F2 or Enter → open label editor for the selected item
      if (event.key === "F2" || (!isTyping && event.key === "Enter")) {
        if (this.selection instanceof Node || this.selection instanceof Edge) {
          event.preventDefault();

          if (this.rightSidebar.classList.contains("wf-collapsed")) {
            this._toggleSidebar(this.rightSidebar);
          }

          await this._waitForRender(this.selection.cell ?? this.selection.link);

          const panel =
            this.selection instanceof Node
              ? this.nodePropertiesPanel
              : this.edgePropertiesPanel;
          const labelInput = panel.querySelector('[data-prop="label"]');
          if (labelInput) {
            labelInput.focus();
            labelInput.select();
          }
        }
        return;
      }

      if (isTyping) {
        return;
      }

      if (event.key === "Escape") {
        event.preventDefault();
        this._deselectAll();
        return;
      }

      if (event.key === "Delete" || event.key === "Backspace") {
        event.preventDefault();
        this._deleteSelected();
        return;
      }

      if (event.key === "-" && !event.ctrlKey && !event.metaKey) {
        event.preventDefault();
        this.zoomOut();
        return;
      }

      if (
        (event.key === "=" || event.key === "+") &&
        !event.ctrlKey &&
        !event.metaKey
      ) {
        event.preventDefault();
        this.zoomIn();
        return;
      }

      // ctrl/cmd + C → copy
      if ((event.ctrlKey || event.metaKey) && event.key === "c") {
        if (this.selection instanceof Node) {
          event.preventDefault();
          this.clipboard = this.selection;
        }
        return;
      }

      // ctrl/cmd + V → paste
      if ((event.ctrlKey || event.metaKey) && event.key === "v") {
        if (this.clipboard) {
          event.preventDefault();

          const bbox = this.clipboard.cell.getBBox();
          const openPosition = this._findOpenPosition(
            bbox.x + 40,
            bbox.y + 40,
            bbox.width,
            bbox.height,
          );
          const clonedCell = this.clipboard.cell.clone();

          clonedCell.position(openPosition.x, openPosition.y);
          clonedCell.addTo(this.graph);
          this._deselectAll();

          const copy = new this.clipboard.constructor(this.clipboard.label);
          copy.cell = clonedCell;
          copy.editor = this;
          this.nodeMap.set(clonedCell.id, copy);

          (async () => {
            await this._waitForRender(clonedCell);
            await this._resizeNodeAsync(clonedCell);
            await this._waitForRender(clonedCell);
            copy.on("change", (changedNode) =>
              this.emit("node:change", changedNode),
            );
            copy.on("move", (movedNode) => this.emit("node:move", movedNode));
            this._selectItem(copy);
          })();

          this.emit("node:add", copy);
        }
        return;
      }

      // arrow keys → nudge the selected node or pan the canvas
      if (
        ["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(event.key)
      ) {
        event.preventDefault();

        const deltaX =
          event.key === "ArrowLeft"
            ? -this.gridSize
            : event.key === "ArrowRight"
              ? this.gridSize
              : 0;
        const deltaY =
          event.key === "ArrowUp"
            ? -this.gridSize
            : event.key === "ArrowDown"
              ? this.gridSize
              : 0;

        if (this.selection instanceof Node) {
          this.selection.moveBy(deltaX, deltaY);
        } else if (!this.selection) {
          const translation = this.renderer.translate();
          this.renderer.translate(
            translation.tx - deltaX,
            translation.ty - deltaY,
          );
        }
      }
    });
  }

  /**
   * Internal: handle touch gestures for mobile pan and pinch-zoom
   */
  _attachTouchListeners() {
    this.renderer.el.addEventListener(
      "touchstart",
      (event) => {
        if (!this._isMobile()) {
          return;
        }

        if (event.touches.length === 1) {
          const touch = event.touches[0];

          // let JointJS handle touches that land on a node
          if (
            document
              .elementFromPoint(touch.clientX, touch.clientY)
              ?.closest(".joint-cell")
          ) {
            return;
          }

          event.stopPropagation();
          this.touchState = {
            type: "pan",
            startX: touch.clientX,
            startY: touch.clientY,
            initialTranslation: this.renderer.translate(),
          };
        } else if (event.touches.length === 2) {
          event.stopPropagation();

          const touch1 = event.touches[0];
          const touch2 = event.touches[1];
          this.touchState = {
            type: "pinch",
            initialDistance: this._touchDistance(touch1, touch2),
            initialMidpoint: this._touchMidpoint(touch1, touch2),
            initialScale: this.renderer.scale().sx,
            initialTranslation: this.renderer.translate(),
          };
        }
      },
      { passive: true },
    );

    this.renderer.el.addEventListener(
      "touchmove",
      (event) => {
        if (!this._isMobile() || !this.touchState) {
          return;
        }
        event.preventDefault();

        const state = this.touchState;

        if (state.type === "pan" && event.touches.length === 1) {
          const touch = event.touches[0];
          this.renderer.translate(
            state.initialTranslation.tx + (touch.clientX - state.startX),
            state.initialTranslation.ty + (touch.clientY - state.startY),
          );
        } else if (state.type === "pinch" && event.touches.length === 2) {
          const touch1 = event.touches[0];
          const touch2 = event.touches[1];
          const newScale = Math.max(
            0.1,
            Math.min(
              10,
              (state.initialScale * this._touchDistance(touch1, touch2)) /
                state.initialDistance,
            ),
          );
          const midpoint = this._touchMidpoint(touch1, touch2);
          const rect = this.renderer.el.getBoundingClientRect();
          const viewportX = midpoint.x - rect.left;
          const viewportY = midpoint.y - rect.top;
          const originX =
            (viewportX - state.initialTranslation.tx) / state.initialScale;
          const originY =
            (viewportY - state.initialTranslation.ty) / state.initialScale;

          this.renderer.scale(newScale, newScale);
          this.renderer.translate(
            viewportX - originX * newScale,
            viewportY - originY * newScale,
          );
        }
      },
      { passive: false },
    );

    this.renderer.el.addEventListener(
      "touchend",
      (event) => {
        if (!this._isMobile()) {
          return;
        }

        // lifting one finger during a pinch transitions back to panning
        if (this.touchState?.type === "pinch" && event.touches.length === 1) {
          const touch = event.touches[0];
          this.touchState = {
            type: "pan",
            startX: touch.clientX,
            startY: touch.clientY,
            initialTranslation: this.renderer.translate(),
          };
        } else {
          this.touchState = null;
        }
      },
      { passive: true },
    );
  }

  /**
   * Internal: select a node or edge and show its properties
   */
  _selectItem(item) {
    this.selection = item;
    const isNode = item instanceof Node;
    const model = isNode ? item.cell : item.link;
    const view = this.renderer.findViewByModel(model);

    if (view) {
      view.el.classList.add("wf-selected");
      joint.highlighters.stroke.add(view, "root", "selection", {
        padding: 5,
        attrs: { stroke: "#3498db", "stroke-width": 3 },
      });
    }

    this.propertiesHeaderActions.style.display = "flex";
    this.propertiesHeaderActions.querySelector(
      '[data-action="duplicate"]',
    ).style.display = isNode ? "flex" : "none";

    if (isNode) {
      this._showPropertiesPanel("node");
      this._fillNodeProperties(item);
    } else {
      this._showPropertiesPanel("edge");
      this._fillEdgeProperties(item);
    }

    this.emit("selection:change", item);
    this._updateMobileButtonVisibility();
  }

  /**
   * Internal: clear the current selection
   */
  _deselectAll() {
    if (this.selection) {
      const model =
        this.selection instanceof Node
          ? this.selection.cell
          : this.selection.link;
      this.renderer.findViewByModel(model)?.el.classList.remove("wf-selected");
    }

    this.renderer.removeTools();
    joint.highlighters.stroke.removeAll(this.renderer);
    this.selection = null;
    this.propertiesHeaderActions.style.display = "none";
    this._showPropertiesPanel("none");
    this.emit("selection:change", null);
    this.canvasArea.focus();
    this._updateMobileButtonVisibility();
  }

  /**
   * Internal: switch which properties panel is visible
   */
  _showPropertiesPanel(which) {
    this.nodePropertiesPanel.style.display = which === "node" ? "flex" : "none";
    this.edgePropertiesPanel.style.display = which === "edge" ? "flex" : "none";
    this.noSelectionMessage.style.display = which === "none" ? "block" : "none";
  }

  /**
   * Internal: populate the node properties panel with the given node's values
   */
  _fillNodeProperties(node) {
    const panel = this.nodePropertiesPanel;
    const setField = (prop, value) => {
      const element = panel.querySelector(`[data-prop="${prop}"]`);
      if (element) {
        element.value = value ?? "";
      }
    };

    setField("label", node.label);
    setField("labelFontSize", node.labelFontSize);
    setField("description", node.description);
    setField("imageUrl", node.imageUrl);
    setField("imageWidth", node.imageWidth);
    setField("imageHeight", node.imageHeight);
    setField("backgroundColor", node.backgroundColor);
    setField("backgroundColorHex", node.backgroundColor);
    setField("borderColor", node.borderColor);
    setField("borderColorHex", node.borderColor);
    setField("labelColor", node.labelColor);
    setField("labelColorHex", node.labelColor);
    setField("descriptionColor", node.descriptionColor);
    setField("descriptionColorHex", node.descriptionColor);

    // remove any previous custom fields before rebuilding them
    panel.querySelector(".wf-custom-props")?.remove();
    const schema = node.getSchema?.() ?? {};
    if (!Object.keys(schema).length) {
      return;
    }

    const customSection = this._makeElement("div", "wf-custom-props");
    const divider = customSection.appendChild(this._makeElement("div"));
    divider.style.cssText = "border-top:1px solid #dee2e6; margin:4px 0;";

    Object.entries(schema).forEach(([key, fieldDefinition]) => {
      if (fieldDefinition.visible === false) {
        return;
      }

      const group = customSection.appendChild(
        this._makeElement("div", "wf-prop-group"),
      );
      const label = group.appendChild(this._makeElement("label"));
      label.textContent = fieldDefinition.label ?? key;

      const currentValue = node.getCustomProperty(key);
      let input;

      if (fieldDefinition.type === "choice") {
        input = this._makeElement("select");
        Object.entries(fieldDefinition.choices ?? {}).forEach(
          ([value, label]) => {
            const option = input.appendChild(this._makeElement("option"));
            option.value = value;
            option.textContent = label;
          },
        );
        input.value = currentValue ?? fieldDefinition.default;
      } else if (fieldDefinition.type === "textarea") {
        input = this._makeElement("textarea");
        input.value = currentValue ?? "";
      } else if (fieldDefinition.type === "boolean") {
        input = this._makeElement("input");
        input.type = "checkbox";
        input.checked = currentValue ?? fieldDefinition.default ?? false;
        input.style.cssText =
          "width:auto !important; margin:0; cursor:pointer; align-self:flex-start;";
        group.appendChild(input);
      } else if (fieldDefinition.type === "color") {
        // color fields get both a colour picker and a hex text input
        const row = group.appendChild(this._makeElement("div", "wf-color-row"));
        const picker = this._makeElement("input");
        picker.type = "color";
        picker.value = currentValue ?? fieldDefinition.default ?? "#000000";
        const hexInput = this._makeElement("input");
        hexInput.type = "text";
        hexInput.value = currentValue ?? fieldDefinition.default ?? "#000000";

        picker.addEventListener("input", () => {
          hexInput.value = picker.value;
          node.setCustomProperty(key, picker.value);
        });
        hexInput.addEventListener("input", () => {
          picker.value = hexInput.value;
          node.setCustomProperty(key, hexInput.value);
        });

        row.appendChild(picker);
        row.appendChild(hexInput);
        group.appendChild(row);
        return;
      } else {
        input = this._makeElement("input");
        input.type = fieldDefinition.type === "number" ? "number" : "text";
        input.value = currentValue ?? "";
        if (fieldDefinition.min !== undefined) {
          input.min = fieldDefinition.min;
        }
        if (fieldDefinition.max !== undefined) {
          input.max = fieldDefinition.max;
        }
      }

      if (fieldDefinition.readonly) {
        input.disabled = true;
      }
      group.appendChild(input);

      input.addEventListener(
        fieldDefinition.type === "boolean" ? "change" : "input",
        () => {
          let newValue;
          if (fieldDefinition.type === "boolean") {
            newValue = input.checked;
          } else if (fieldDefinition.type === "number") {
            newValue = parseFloat(input.value);
          } else {
            newValue = input.value;
          }
          node.setCustomProperty(key, newValue);
        },
      );
    });

    panel.appendChild(customSection);
  }

  /**
   * Internal: populate the edge properties panel with the given edge's values
   */
  _fillEdgeProperties(edge) {
    const panel = this.edgePropertiesPanel;
    const setField = (prop, value) => {
      const element = panel.querySelector(`[data-prop="${prop}"]`);
      if (element) {
        element.value = value ?? "";
      }
    };

    setField("label", edge.label);
    setField("labelFontSize", edge.labelFontSize);
    setField("labelColor", edge.labelColor);
    setField("labelColorHex", edge.labelColor);
    setField("lineColor", edge.lineColor);
    setField("lineColorHex", edge.lineColor);
    setField("lineStyle", edge.lineStyle);
    setField("connectorType", edge.connectorType);
    setField("sourceArrow", edge.sourceArrow);
    setField("targetArrow", edge.targetArrow);
    setField("description", edge.description);
  }

  /**
   * Internal: collapse or expand a sidebar
   */
  _toggleSidebar(sidebar) {
    const isLeft = sidebar === this.leftSidebar;
    const icon = isLeft ? this.leftCollapseIcon : this.rightCollapseIcon;
    const previousTranslation = this.renderer.translate();
    const isNowCollapsed = sidebar.classList.toggle("wf-collapsed");

    icon.style.transform = isNowCollapsed ? "rotate(180deg)" : "rotate(0deg)";
    sidebar.querySelector(".wf-collapsed-label").style.display = isNowCollapsed
      ? "block"
      : "none";

    // on mobile, opening one sidebar closes the other
    if (this._isMobile() && !isNowCollapsed) {
      const otherSidebar = isLeft ? this.rightSidebar : this.leftSidebar;
      const otherIcon = isLeft ? this.rightCollapseIcon : this.leftCollapseIcon;
      if (!otherSidebar.classList.contains("wf-collapsed")) {
        otherSidebar.classList.add("wf-collapsed");
        otherIcon.style.transform = "rotate(180deg)";
        otherSidebar.querySelector(".wf-collapsed-label").style.display =
          "block";
      }
    }

    // continuously resize the renderer during the CSS transition
    const interval = setInterval(() => {
      const area = this.canvasArea;
      this.renderer.setDimensions(area.clientWidth, area.clientHeight);
      this.renderer.translate(previousTranslation.tx, previousTranslation.ty);
    }, 16);
    setTimeout(() => clearInterval(interval), 350);

    this._updateMobileButtonVisibility();
  }

  /**
   * Internal: set a sidebar's collapsed state directly (no toggle)
   */
  _setSidebarCollapsed(sidebar, collapsed) {
    const isLeft = sidebar === this.leftSidebar;
    const icon = isLeft ? this.leftCollapseIcon : this.rightCollapseIcon;

    sidebar.classList.toggle("wf-collapsed", collapsed);
    icon.style.transform = collapsed ? "rotate(180deg)" : "rotate(0deg)";
    sidebar.querySelector(".wf-collapsed-label").style.display = collapsed
      ? "block"
      : "none";
  }

  /**
   * Internal: collapse all sidebars on mobile (e.g. when tapping blank canvas)
   */
  _collapseAllSidebarsOnMobile() {
    if (!this._isMobile()) {
      return;
    }

    const previousTranslation = this.renderer.translate();
    this._setSidebarCollapsed(this.leftSidebar, true);
    this._setSidebarCollapsed(this.rightSidebar, true);

    const interval = setInterval(() => {
      const area = this.canvasArea;
      this.renderer.setDimensions(area.clientWidth, area.clientHeight);
      this.renderer.translate(previousTranslation.tx, previousTranslation.ty);
    }, 16);
    setTimeout(() => clearInterval(interval), 350);

    this._updateMobileButtonVisibility();
  }

  /**
   * Internal: duplicate the currently selected node
   */
  _duplicateSelected() {
    if (!(this.selection instanceof Node)) {
      return;
    }

    const bbox = this.selection.cell.getBBox();
    const openPosition = this._findOpenPosition(
      bbox.x + 40,
      bbox.y + 40,
      bbox.width,
      bbox.height,
    );
    const clonedCell = this.selection.cell.clone();

    clonedCell.translate(openPosition.x - bbox.x, openPosition.y - bbox.y);
    clonedCell.addTo(this.graph);

    const copy = new this.selection.constructor(this.selection.label);
    copy.cell = clonedCell;
    copy.editor = this;
    this.nodeMap.set(clonedCell.id, copy);
    this._selectItem(copy);
    this.emit("node:add", copy);
  }

  /**
   * Internal: delete the currently selected node or edge
   */
  _deleteSelected() {
    if (!this.selection) {
      return;
    }
    const item = this.selection;
    this._deselectAll();
    item.remove();
  }

  /**
   * Internal: pan and zoom the camera to centre on the selected item
   */
  _focusCameraOnSelection() {
    if (!this.selection) {
      return;
    }
    const model =
      this.selection instanceof Node
        ? this.selection.cell
        : this.selection.link;
    const center = model.getBBox().center();
    const size = this.renderer.getComputedSize();

    this.renderer.scale(1, 1);
    this.renderer.translate(
      size.width / 2 - center.x,
      size.height / 2 - center.y,
    );
  }

  /**
   * Internal: create an edge between two nodes
   */
  _createEdge(sourceNode, targetNode, sourcePortIndex, targetPortIndex) {
    // disallow a reverse edge if one already exists in the other direction
    const reverseExists = [...this.edgeMap.values()].some(
      (edge) => edge.source === targetNode && edge.target === sourceNode,
    );
    if (reverseExists) {
      return null;
    }

    const link = new joint.shapes.standard.Link({
      attrs: {
        line: {
          stroke: "#495057",
          strokeWidth: 2,
          targetMarker: ARROW_MARKERS.classic,
        },
      },
    });

    // resolve port indices to actual port objects
    const sourcePorts = sourceNode.cell.getPorts();
    const targetPorts = targetNode.cell.getPorts();
    const sourcePort =
      sourcePortIndex !== null ? sourcePorts[sourcePortIndex - 1] : null;
    const targetPort =
      targetPortIndex !== null ? targetPorts[targetPortIndex - 1] : null;

    link.source(
      sourcePort
        ? { id: sourceNode.cell.id, port: sourcePort.id }
        : { id: sourceNode.cell.id },
    );
    link.target(
      targetPort
        ? { id: targetNode.cell.id, port: targetPort.id }
        : { id: targetNode.cell.id },
    );

    if (sourcePortIndex !== null) {
      link.set("sourcePort", sourcePortIndex);
    }
    if (targetPortIndex !== null) {
      link.set("targetPort", targetPortIndex);
    }

    this.graph.startBatch("edge");
    link.addTo(this.graph);
    this.graph.stopBatch("edge");
    this.renderer.updateViews();

    const edge = new Edge(link, sourceNode, targetNode, this);
    this.edgeMap.set(link.id, edge);
    edge.on("change", (changedEdge) => this.emit("edge:change", changedEdge));
    this.emit("edge:add", edge);
    return edge;
  }

  /**
   * Internal: return all edges connected to a given node
   */
  _getEdgesForNode(node) {
    return [...this.edgeMap.values()].filter(
      (edge) => edge.source === node || edge.target === node,
    );
  }

  /**
   * Internal: re-route auto-managed ports for all edges connected to a cell
   */
  _updateConnectionPorts(cell) {
    if (!this.autoPortsOn || this.isLoading) {
      return;
    }

    this.graph.getConnectedLinks(cell).forEach((link) => {
      // skip links that have a manually pinned port on either end
      if (typeof link.get("sourcePort") === "number") {
        return;
      }
      if (typeof link.get("targetPort") === "number") {
        return;
      }

      const sourceCell = link.getSourceElement();
      const targetCell = link.getTargetElement();
      if (!sourceCell || !targetCell) {
        return;
      }

      // returns the centre point of a cell in graph coordinates
      const getCenterOf = (element) => {
        const position = element.position();
        const size = element.size();
        return {
          x: position.x + size.width / 2,
          y: position.y + size.height / 2,
        };
      };

      // returns the port ID whose angle most closely faces towardPoint
      const getBestPortFacing = (element, towardPoint) => {
        const center = getCenterOf(element);
        const angleToward = Math.atan2(
          towardPoint.y - center.y,
          towardPoint.x - center.x,
        );
        const portPositions = element.getPortsPositions("all");
        const size = element.size();
        let bestPortId = null;
        let bestAngleDiff = Infinity;

        element.getPorts().forEach((port) => {
          const position = portPositions[port.id];
          if (!position) {
            return;
          }
          const portAngle = Math.atan2(
            position.y - size.height / 2,
            position.x - size.width / 2,
          );
          let angleDiff = Math.abs(angleToward - portAngle);
          if (angleDiff > Math.PI) {
            angleDiff = 2 * Math.PI - angleDiff;
          }
          if (angleDiff < bestAngleDiff) {
            bestAngleDiff = angleDiff;
            bestPortId = port.id;
          }
        });
        return bestPortId;
      };

      const sourceCenter = getCenterOf(sourceCell);
      const targetCenter = getCenterOf(targetCell);
      const bestSourcePort = getBestPortFacing(sourceCell, targetCenter);
      const bestTargetPort = getBestPortFacing(targetCell, sourceCenter);

      if (link.source().port !== bestSourcePort) {
        link.source({ id: sourceCell.id, port: bestSourcePort });
        link.unset("sourcePort");
      }
      if (link.target().port !== bestTargetPort) {
        link.target({ id: targetCell.id, port: bestTargetPort });
        link.unset("targetPort");
      }
    });
  }

  /**
   * Internal: return a promise that resolves once a cell's view is in the DOM
   */
  _waitForRender(cell) {
    return new Promise((resolve) => {
      const view = this.renderer.findViewByModel(cell);
      if (!view) {
        resolve();
        return;
      }
      // two rAF passes ensure the browser has fully painted the cell
      requestAnimationFrame(() => requestAnimationFrame(() => resolve(view)));
    });
  }

  /**
   * Internal: resize a cell asynchronously after it has been rendered
   */
  async _resizeNodeAsync(cell) {
    this._fitNodeToContent(cell);
    await this._waitForRender(cell);
    this.renderer.updateViews();
  }

  /**
   * Internal: measure the cell's content and resize it to fit
   */
  _fitNodeToContent(cell) {
    const view = this.renderer.findViewByModel(cell);
    if (!view) {
      return;
    }

    const labelElement = view.el.querySelector(".label");
    const descriptionElement = view.el.querySelector(".descriptionLabel");
    if (!labelElement || !descriptionElement) {
      return;
    }

    // scale font sizes according to the labelFontSize percentage
    const fontScale = (cell.get("fontSizePercent") || 100) / 100;
    cell.attr({
      label: { fontSize: 13 * fontScale },
      descriptionLabel: { fontSize: 11 * fontScale },
    });

    const imageUrl = cell.get("imageUrl");
    const shapeType = cell.get("type");
    const descriptionText = cell.attr("descriptionLabel/text") || "";
    const imageWidth = parseInt(cell.get("imageWidth") || 32);
    const imageHeight = parseInt(cell.get("imageHeight") || 32);
    const padding = 15;
    const imageSpacing = imageUrl ? 10 : 0;

    // measure the rendered text bounding boxes
    const labelBBox = labelElement.getBBox();
    const descriptionBBox = descriptionElement.getBBox();
    const textWidth = Math.max(labelBBox.width, descriptionBBox.width);
    const totalContentWidth =
      (imageUrl ? imageWidth : 0) + imageSpacing + textWidth;

    // calculate the minimum size needed to fit all content
    let width = Math.max(140, padding + totalContentWidth + padding);
    const totalTextHeight = descriptionText
      ? labelBBox.height + descriptionBBox.height
      : labelBBox.height;
    let height = Math.max(
      50,
      padding + Math.max(imageUrl ? imageHeight : 0, totalTextHeight) + padding,
    );

    // certain shapes have aspect ratio constraints
    if (shapeType === "diamond" || shapeType === "circle") {
      width = height = Math.max(width, height);
    } else if (
      ["triangle", "hexagon", "pentagon", "octagon"].includes(shapeType)
    ) {
      if (width / height > 1.2) {
        height = width / 1.2;
      } else {
        width = height * 1.2;
      }
      if (shapeType === "triangle") {
        width *= 1.25;
        height *= 1.25;
      }
    }

    // position the image and text content within the cell
    const contentStartX = (width - totalContentWidth) / 2;
    const textCenterX =
      contentStartX +
      (imageUrl ? imageWidth : 0) +
      imageSpacing +
      textWidth / 2;

    cell.attr("image", {
      refX: 0,
      refX2: contentStartX,
      refY: 0.5,
      y: -imageHeight / 2,
      width: imageWidth,
      height: imageHeight,
    });

    if (descriptionText) {
      const contentStartY = (height - totalTextHeight) / 2;
      cell.attr("label", {
        refX: 0,
        refX2: textCenterX,
        refY: 0,
        refY2: contentStartY + labelBBox.height / 2,
        textAnchor: "middle",
        textVerticalAnchor: "middle",
      });
      cell.attr("descriptionLabel", {
        refX: 0,
        refX2: textCenterX,
        refY: 0,
        refY2: contentStartY + labelBBox.height + descriptionBBox.height / 2,
        textAnchor: "middle",
        textVerticalAnchor: "middle",
      });
    } else {
      cell.attr("label", {
        refX: 0,
        refX2: textCenterX,
        refY: 0.5,
        refY2: 0,
        textAnchor: "middle",
        textVerticalAnchor: "middle",
      });
    }

    cell.resize(width, height);
  }

  /**
   * Internal: returns true if the user is on a touch-only mobile device
   */
  _isMobile() {
    return window.matchMedia("(pointer:coarse) and (max-width:767px)").matches;
  }

  /**
   * Internal: create a DOM element with an optional CSS class
   */
  _makeElement(tag, className = "") {
    const element = document.createElement(tag);
    if (className) {
      element.className = className;
    }
    return element;
  }

  /**
   * Internal: create and append a zoom button to a parent element
   */
  _makeZoomButton(parent, title, html) {
    const button = parent.appendChild(
      this._makeElement("button", "wf-ctrl-btn"),
    );
    button.title = title;
    button.innerHTML = html;
    return button;
  }

  /**
   * Internal: zoom the canvas around a specific screen point
   */
  _zoomAtPoint(factor, clientX, clientY) {
    const currentScale = this.renderer.scale().sx;
    const newScale = Math.max(0.1, Math.min(10, currentScale * factor));
    const rect = this.renderer.el.getBoundingClientRect();
    const viewportX = clientX - rect.left;
    const viewportY = clientY - rect.top;
    const translation = this.renderer.translate();

    // keep the point under the cursor stationary as we scale
    const originX = (viewportX - translation.tx) / currentScale;
    const originY = (viewportY - translation.ty) / currentScale;
    this.renderer.scale(newScale, newScale);
    this.renderer.translate(
      viewportX - originX * newScale,
      viewportY - originY * newScale,
    );
  }

  /**
   * Internal: zoom the canvas around its visible centre point
   */
  _zoomAtCenter(factor) {
    const rect = this.renderer.el.getBoundingClientRect();
    this._zoomAtPoint(
      factor,
      rect.left + rect.width / 2,
      rect.top + rect.height / 2,
    );
  }

  /**
   * Internal: find the nearest grid-aligned position that doesn't overlap any existing node
   */
  _findOpenPosition(startX, startY, width, height, excludeCell = null) {
    const clearance = this.clearanceUnits * this.gridSize;

    // snap the starting position to the grid
    let x = Math.round(startX / this.gridSize) * this.gridSize;
    let y = Math.round(startY / this.gridSize) * this.gridSize;

    const isBlocked = (testX, testY) => {
      const testRect = new joint.g.Rect(testX, testY, width, height).inflate(
        clearance,
      );
      return this.graph.getElements().some((element) => {
        if (excludeCell && element.id === excludeCell.id) {
          return false;
        }
        return testRect.intersect(element.getBBox().clone().inflate(clearance));
      });
    };

    if (!isBlocked(x, y)) {
      return { x, y };
    }

    // spiral outward until we find a clear spot
    for (let radius = 1; radius < 30; radius++) {
      for (let deltaX = -radius; deltaX <= radius; deltaX++) {
        for (let deltaY = -radius; deltaY <= radius; deltaY++) {
          // only test the perimeter of the current radius
          if (Math.abs(deltaX) !== radius && Math.abs(deltaY) !== radius) {
            continue;
          }
          const testX = x + deltaX * this.gridSize;
          const testY = y + deltaY * this.gridSize;
          if (!isBlocked(testX, testY)) {
            return { x: testX, y: testY };
          }
        }
      }
    }

    // fall back to the original position if nothing was found
    return { x, y };
  }

  /**
   * Internal: calculate the distance between two touch points
   */
  _touchDistance(touch1, touch2) {
    const deltaX = touch1.clientX - touch2.clientX;
    const deltaY = touch1.clientY - touch2.clientY;
    return Math.sqrt(deltaX * deltaX + deltaY * deltaY);
  }

  /**
   * Internal: calculate the midpoint between two touch points
   */
  _touchMidpoint(touch1, touch2) {
    return {
      x: (touch1.clientX + touch2.clientX) / 2,
      y: (touch1.clientY + touch2.clientY) / 2,
    };
  }
}
