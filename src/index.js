...
class PathPoint extends EventBus {
    ...
    moveTo(x, y) {
        const points = [...this._edge.vertices()];
...
DiagramNode.define = function (BaseNodeClass, defaultOptions = {}, schema = {}, renderFn = null) {
    ...
    Object.defineProperty(CustomNode.prototype, "portCount", Object.getOwnPropertyDescriptor(BaseNodeClass.prototype, "portCount") || {
        get() {
            return 4;
        },
    });
...
function _buildPolygonCell(position, namespace, shapeType, width, height, points) {
...
class DiagramEditor extends EventBus {
    ...
    registerNodeType(label, NodeClass) {
        const item = this._shapeLibrary.appendChild(this._makeElement("div", "wf-node-template"));
        ...
        item.addEventListener("dragstart", (event) => event.dataTransfer.setData("customNode", label));
    ...
    addNode(node, canvasX, canvasY) {
        ...
        const localPosition = this._renderer.clientToLocalPoint({
            x: centerX + area.getBoundingClientRect().left,
            y: centerY + area.getBoundingClientRect().top,
        });
        const isSquarish = node instanceof Square ||
            node instanceof Circle ||
            node instanceof Diamond;
        const width = isSquarish ? 80 : 140;
        const height = isSquarish ? 80 : 50;
        const openPosition = this._findOpenPosition(localPosition.x - width / 2, localPosition.y - height / 2, width, height);
    ...
    centerContent() {
        ...
        this._renderer.translate(size.width / 2 - (bbox.x + bbox.width / 2), size.height / 2 - (bbox.y + bbox.height / 2));
        return this;
    }
    zoomIn(factor = 1.25) {
    ...
    serialize() {
        const nodes = [...this._nodeMap.values()].map((node) => ({
            id: node.cell.id,
        ...
        const edges = [...this._edgeMap.values()].map((edge) => {
    ...
    async deserialize(json) {
        ...
        const nodeClassMap = {
            ...
            Pentagon,
            Octagon,
            ...Object.fromEntries(Object.entries(this._registeredNodeTypes).map(([, cls]) => [
                cls.name,
                cls,
            ])),
        };
        ...
        this._graph.getLinks().forEach((link) => {
            ...
            if (!edge)
                return;
            const savedEdge = edgeDataList.find((data) => oldIdToNode[data.sourceId]?.cell.id === edge.source.cell.id &&
    ...
    _selectItem(item) {
        ...
        this._propertiesHeaderActions.querySelector('[data-action="duplicate"]').style.display = isNode ? "flex" : "none";
        if (isNode) {
    ...
    _getEdgesForNode(node) {
        return [...this._edgeMap.values()].filter((edge) => edge.source === node || edge.target === node);
    }
    _createEdge(sourceNode, targetNode, sourcePortIndex, targetPortIndex) {
        const reverseExists = [...this._edgeMap.values()].some((edge) => edge.source === targetNode && edge.target === sourceNode);
        if (reverseExists)
            return null;
    ...
    _fitNodeToContent(cell) {
        ...
        const textCenterX = contentStartX +
    ...
    _updateMobileButtonVisibility() {
        if (!this._isMobile())
            return;
        const shouldHide = this._selection !== null ||
    ...
    _buildLayout() {
      ...
      <span class="wf-collapsed-label" style="display:none">Library</span>`;
        this._leftCollapseButton = leftHeader.querySelector(".wf-collapse-btn");
        ...
        this._exportButton = this._topRightButtons.appendChild(this._makeElement("button", "wf-ctrl-btn"));
        ...
        this._importButton = this._topRightButtons.appendChild(this._makeElement("button", "wf-ctrl-btn"));
        ...
        this._importFileInput = this._canvasArea.appendChild(this._makeElement("input"));
        ...
        this._autoPortToggleButton = this._topLeftButtons.appendChild(this._makeElement("button", "wf-ctrl-btn active"));
        ...
        this._autoArrangeButton = this._topLeftButtons.appendChild(this._makeElement("button", "wf-ctrl-btn"));
      ...
      <div class="wf-header-actions wf-sidebar-body" style="display:none;">
        ...
        this._rightCollapseButton = rightHeader.querySelector(".wf-collapse-btn");
        this._rightCollapseIcon = rightHeader.querySelector(".wf-collapse-icon");
        this._propertiesHeaderActions = rightHeader.querySelector(".wf-header-actions");
        this._nodePropertiesPanel = this._buildNodePropertiesPanel();
        this._edgePropertiesPanel = this._buildEdgePropertiesPanel();
        this._noSelectionMessage = this._rightSidebar.appendChild(this._makeElement("div", "wf-no-selection wf-sidebar-body"));
    ...
    _buildShapeLibrary() {
        ...
        builtInShapes.forEach(({ label, type, cls }) => {
            ...
            item.addEventListener("dragstart", (event) => event.dataTransfer.setData("type", type));
            item.addEventListener("click", () => {
                if (!this._isMobile())
                    return;
    ...
    _buildNodePropertiesPanel() {
        const panel = this._rightSidebar.appendChild(this._makeElement("div", "wf-properties wf-prop-panel-element wf-sidebar-body"));
    ...
    _buildEdgePropertiesPanel() {
        const panel = this._rightSidebar.appendChild(this._makeElement("div", "wf-properties wf-prop-panel-link wf-sidebar-body"));
        panel.style.display = "none";
        panel.innerHTML = `
      <div class="wf-prop-group">
        <label>Label</label>
    ...
    _attachButtonListeners() {
        ...
        this._importFileInput.addEventListener("change", (event) => {
            ...
            reader.onload = (loadEvent) => this.deserialize(loadEvent.target.result).catch((error) => alert("Import failed: " + error.message));
        ...
        this._renderer.el.addEventListener("wheel", (event) => {
        ...
        this._canvasArea.addEventListener("drop", (event) => {
            event.preventDefault();
            ...
            if (customLabel && this._registeredNodeTypes?.[customLabel]) {
                ...
                const dropPosition = this._renderer.clientToLocalPoint({
                    x: event.clientX,
                    y: event.clientY,
                });
                const node = new NodeClass();
        ...
        this._nodePropertiesPanel.querySelector('[data-action="upload-image"]').addEventListener("click", () => {
            this._nodePropertiesPanel.querySelector('[data-action="image-file"]').click();
        });
        this._nodePropertiesPanel.querySelector('[data-action="image-file"]').addEventListener("change", (event) => {
            const file = event.target.files?.[0];
            if (!file)
                return;
            const reader = new FileReader();
            reader.onload = (loadEvent) => {
                this._nodePropertiesPanel.querySelector('[data-prop="imageUrl"]').value = loadEvent.target.result;
                if (this._selection instanceof DiagramNode) {
                    this._selection.imageUrl = loadEvent.target
                        .result;
                }
            };
    ...
    _handleNodePropertyChange(event) {
        ...
        if (input.dataset.pair) {
            this._nodePropertiesPanel.querySelector(`[data-prop="${input.dataset.pair}"]`).value = String(value);
        }
        if (input.dataset.pairPicker) {
            this._nodePropertiesPanel.querySelector(`[data-prop="${input.dataset.pairPicker}"]`).value = String(value);
        }
        const resolvedProp = prop.endsWith("Hex") ? prop.slice(0, -3) : prop;
        if (resolvedProp in this._selection)
            this._selection[resolvedProp] = value;
    }
    _handleEdgePropertyChange(event) {
        ...
        const value = input.type === "number" ? parseFloat(input.value) : input.value;
        if (input.dataset.pair) {
            this._edgePropertiesPanel.querySelector(`[data-prop="${input.dataset.pair}"]`).value = String(value);
        }
        if (input.dataset.pairPicker) {
            this._edgePropertiesPanel.querySelector(`[data-prop="${input.dataset.pairPicker}"]`).value = String(value);
        }
        const resolvedProp = prop.endsWith("Hex") ? prop.slice(0, -3) : prop;
        if (resolvedProp in this._selection)
            this._selection[resolvedProp] = value;
    ...
    _attachDiagramListeners() {
        ...
        this._renderer.on("element:pointerdown", (view, event) => {
        ...
        this._renderer.on("element:pointerup", (view, event) => {
        ...
        this._renderer.on("link:pointerdown", (view) => {
        ...
        this._renderer.on("cell:pointerdown tool:pointerdown", (view, event) => {
        ...
        this._renderer.on("cell:contextmenu", (view, event) => {
            event.preventDefault();
            ...
            this._contextMenu.querySelector('[data-action="duplicate"]').style.display = this._nodeMap.has(view.model.id) ? "flex" : "none";
        });
        this._renderer.on("blank:pointerdown", (event) => {
            ...
            const onMove = (moveEvent) => this._renderer.translate(initialTranslation.tx + (moveEvent.clientX - startX), initialTranslation.ty + (moveEvent.clientY - startY));
            const onUp = (upEvent) => {
        ...
        this._renderer.on("link:connect", (view) => {
    ...
    _attachKeyboardShortcuts() {
        ...
        this._canvasArea.focus();
        this._canvasArea.addEventListener("mousedown", () => {
            const tag = document.activeElement?.tagName;
            if (tag !== "INPUT" && tag !== "TEXTAREA")
                this._canvasArea.focus();
        });
        this._canvasArea.addEventListener("keydown", async (event) => {
            const tag = document.activeElement?.tagName;
            ...
            if (event.key === "F2" || (!isTyping && event.key === "Enter")) {
                if (this._selection instanceof DiagramNode ||
                    ...
                    await this._waitForRender(this._selection instanceof DiagramNode
            ...
            if ((event.ctrlKey || event.metaKey) && event.key === "v") {
                if (this._clipboard) {
                    ...
                    this._deselectAll();
                    const copy = new this._clipboard.constructor({
                    ...
                    (async () => {
                        ...
                        await this._waitForRender(clonedCell);
                        copy.on("change", (changedNode) => this.emit("node:change", changedNode));
    ...
    _attachTouchListeners() {
        this._renderer.el.addEventListener("touchstart", (event) => {
            if (!this._isMobile())
                return;
        ...
        this._renderer.el.addEventListener("touchmove", (event) => {
            ...
            if (state.type === "pan" && event.touches.length === 1) {
                const touch = event.touches[0];
                this._renderer.translate(state.initialTranslation.tx + (touch.clientX - state.startX), state.initialTranslation.ty + (touch.clientY - state.startY));
            }
            else if (state.type === "pinch" && event.touches.length === 2) {
                const touch1 = event.touches[0];
                const touch2 = event.touches[1];
                const newScale = Math.max(0.1, Math.min(10, (state.initialScale * this._touchDistance(touch1, touch2)) /
        ...
        this._renderer.el.addEventListener("touchend", (event) => {
            if (!this._isMobile())
                return;
        ...
        Object.entries(schema).forEach(([key, fieldDef]) => {
            ...
            if (fieldDefinition.type === "choice") {
                ...
                if (fieldDefinition.min !== undefined)
                    input.min = String(fieldDefinition.min);
                if (fieldDefinition.max !== undefined)
                    input.max = String(fieldDefinition.max);
            }
    ...
    _toggleSidebar(sidebar) {
        ...
        icon.style.transform = isNowCollapsed
            ? "rotate(180deg)"
            : "rotate(0deg)";
    ...
    _setSidebarCollapsed(sidebar, collapsed) {
        ...
        icon.style.transform = collapsed
            ? "rotate(180deg)"
            : "rotate(0deg)";
    ...
    _duplicateSelected() {
        ...
        clonedCell.addTo(this._graph);
        const copy = new this._selection.constructor({
    ...
    _updateConnectionPorts(cell) {
        if (!this._autoPortsOn || this._isLoading)
            return;
        this._graph.getConnectedLinks(cell).forEach((link) => {
    ...
    _zoomAtPoint(factor, clientX, clientY) {
        const currentScale = this._renderer.scale().sx;
    ...
    _findOpenPosition(startX, startY, width, height, excludeCell = null) {
        ...
        const isBlocked = (testX, testY) => {
            ...
            return this._graph.getElements().some((element) => {
